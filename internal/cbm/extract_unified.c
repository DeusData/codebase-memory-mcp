...
static bool gdscript_is_global_load_call_node(CBMExtractCtx *ctx, TSNode call_node) {
    ...
    TSNode object_node = ts_node_child_by_field_name(call_node, TS_FIELD("object"));
    if (!ts_node_is_null(object_node)) {
        return false;
    }

    const char *callee = NULL;
    TSNode function_node = ts_node_child_by_field_name(call_node, TS_FIELD("function"));
    if (!ts_node_is_null(function_node)) {
        callee = cbm_node_text(ctx->arena, function_node, ctx->source);
    }
    if (!callee || !callee[0]) {
        TSNode name_node = ts_node_child_by_field_name(call_node, TS_FIELD("name"));
        if (!ts_node_is_null(name_node)) {

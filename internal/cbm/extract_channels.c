...
/* tree-sitter-elixir gives a call's arguments node NO field name — its whole
 * field set is key, left, operand, operator, quoted_start, quoted_end, right,
 * target and value — so the generic field lookup is always null here and every
 * channel below was silently skipped. extract_defs.c has carried the same
 * positional fallback for the definition side since Elixir was added. */
static TSNode elixir_channel_call_args(TSNode call) {
    TSNode args = ts_node_child_by_field_name(call, TS_FIELD("arguments"));
...
/* The callee of an Elixir `call`. The grammar labels it `target`; child(0) is
 * the same node and is what extract_defs.c reads, so accept either. */
static TSNode elixir_channel_call_target(TSNode call) {
    TSNode target = ts_node_child_by_field_name(call, TS_FIELD("target"));
    if (ts_node_is_null(target) && ts_node_child_count(call) > 0) {
        target = ts_node_child(call, 0);
    }
    return target;
}

/* Elixir def* macros. `def`/`defp` cover Phoenix channel callbacks; the macro
 * forms are included because `handle_in` is generated by macros in some
 * libraries. */
static bool elixir_channel_is_def_macro(const char *macro) {
    if (!macro) {
        return false;
    }
    return strcmp(macro, "def") == 0 || strcmp(macro, "defp") == 0 ||
           strcmp(macro, "defmacro") == 0 || strcmp(macro, "defmacrop") == 0;
}

static void elixir_process_call(CBMExtractCtx *ctx, TSNode call) {
    TSNode target = elixir_channel_call_target(call);
    ...
    TSNode args = elixir_channel_call_args(call);
 ...
 * `def_call` is the `call` node of a def* macro. Elixir has no `def` node type
 * and no `name`/`parameters` fields: a definition is a call whose first
 * argument is the head, itself a call of name(params) — wrapped in a `when`
 * binary_operator when the clause carries a guard. */
static void elixir_process_function_def(CBMExtractCtx *ctx, TSNode def_call) {
    TSNode args = elixir_channel_call_args(def_call);
    if (ts_node_is_null(args) || ts_node_named_child_count(args) == 0) {
        return;
    }
    TSNode head = ts_node_named_child(args, 0);
    if (strcmp(ts_node_type(head), "binary_operator") == 0) {
        ...
        if (!ts_node_is_null(op) && !ts_node_is_null(left) &&
            ts_node_end_byte(op) - ts_node_start_byte(op) == 4 &&
            strncmp(ctx->source + ts_node_start_byte(op), "when", 4) == 0) {
            head = left;
        }
    }
    if (strcmp(ts_node_type(head), "call") != 0 || ts_node_child_count(head) == 0) {
        return;
    }
    TSNode name_node = ts_node_child(head, 0);
    ...
    TSNode params = elixir_channel_call_args(head);
    ...
    if (val) {
        push_channel(ctx, val, "phoenix_channel", CBM_CHANNEL_LISTEN, def_call);
    }
}
...
static void extract_channels_elixir(CBMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        TSNode node = ts_nstack_pop(&stack);
        const char *kind = ts_node_type(node);
        /* Everything in Elixir is a `call`, including `def`. The old dispatch
         * tested for a "def" node type the grammar does not have, so the
         * handle_in listener branch was unreachable. */
        if (strcmp(kind, "call") == 0) {
            TSNode target = elixir_channel_call_target(node);
            char *macro =
                ts_node_is_null(target) ? NULL : cbm_node_text(ctx->arena, target, ctx->source);
            if (elixir_channel_is_def_macro(macro)) {

...
- Grammars: **163** — vendored-from-upstream: **143**, first-party/self-maintained: **15**, registry-disagreement: **5** (nim removed 2026-06-12; objectscript_udl + objectscript_routine added 2026-06-24; mojo added 2026-07-01; arkts added 2026-08-26; plsql added 2026-08-27; chialisp added 2026-08-28 — see notes below)
- ABI distribution: **9×** ABI-13 **80×** ABI-14 **74×** ABI-15 (runtime ceiling is ABI 15; never vendor ABI 16 without a runtime upgrade)
...
- **iec_st** (added 2026-08-22): vendored from [HeytalePazguato/tree-sitter-iec61131-3-st](https://github.com/HeytalePazguato/tree-sitter-iec61131-3-st) @ `00e24f50f8de` (v0.1.2) — MIT, ABI 15. Standard-first IEC 61131-3 (3rd edition) Structured Text grammar with OOP support (METHOD/PROPERTY/EXTENDS/IMPLEMENTS) and case-insensitive keywords; a niche automation-vendor language, hence `vendor-maintained` (not in nvim-treesitter/Helix). Vendored surface is `src/parser.c`, `src/scanner.c`, `src/tree_sitter/*.h`, and upstream LICENSE only — byte-for-byte upstream, no local patches. Known dialect gaps accepted at vendoring time (localized ERROR-recovery, non-fatal): TwinCAT-style `END_STRUCT` without trailing `;`, `REFERENCE TO`, and `AT %I*` wildcard addresses. **Forked 2026-08-23** into `tools/tree-sitter-iec-st/` (grammar.js + generated parser committed, same layout as tools/tree-sitter-form) because the upstream grammar is standard-only and real TwinCAT projects did not parse: measured against a 130-file production solution, 49 files parsed partially before the fork and 8 after. The fork adds, each verified against that corpus: access/inheritance modifiers AFTER the POU keyword (`FUNCTION_BLOCK PUBLIC FINAL FB_X`), wildcard located addresses (`AT %I*`, spaced or glued, on VAR entries and DUT fields), bit-in-word member access (`wError.0 := TRUE`), a base type after an enumerator list (`(Idle, Run) UINT;`), `REFERENCE TO` beside the standard `REF_TO`, function-block instance argument lists (`fb : FB_T()`, `ByteBuffer(ADR(x), SIZEOF(x))`), an optional terminator after a block-shaped TYPE definition, attribute pragmas above POU members, TwinCAT `__TRY/__CATCH/__FINALLY/__ENDTRY`, interface method prototypes terminated by `;`, and tolerance for a stray `;` in a VAR block. Regenerate with `tree-sitter generate` (CLI 0.26.x, ABI 15) inside tools/tree-sitter-iec-st and copy src/parser.c + src/scanner.c here. Upstream re-vendoring must re-apply these dialect rules.
...
| iec_st | 15 | HeytalePazguato/tree-sitter-iec61131-3-st | `00e24f50f8de` | self-maintained fork (tools/tree-sitter-iec-st) | ✅ |

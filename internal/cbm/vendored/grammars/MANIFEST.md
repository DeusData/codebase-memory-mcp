...
- **iec_st** (added 2026-08-22): vendored from [HeytalePazguato/tree-sitter-iec61131-3-st](https://github.com/HeytalePazguato/tree-sitter-iec61131-3-st) @ `00e24f50f8de` (v0.1.2) — MIT, ABI 15. Standard-first IEC 61131-3 (3rd edition) Structured Text grammar with OOP support (METHOD/PROPERTY/EXTENDS/IMPLEMENTS) and case-insensitive keywords; a niche automation-vendor language, hence `vendor-maintained` (not in nvim-treesitter/Helix). Vendored surface is `src/parser.c`, `src/scanner.c`, `src/tree_sitter/*.h`, and upstream LICENSE only — byte-for-byte upstream, no local patches. Known dialect gaps accepted at vendoring time (localized ERROR-recovery, non-fatal): TwinCAT-style `END_STRUCT` without trailing `;`, `REFERENCE TO`, and `AT %I*` wildcard addresses.
...
| iec_st | 15 | HeytalePazguato/tree-sitter-iec61131-3-st | `00e24f50f8de` | vendor-maintained | ✅ |

...
- **sql (refreshed 2026-09-20):** regenerated from [DerekStride/tree-sitter-sql](https://github.com/DerekStride/tree-sitter-sql) @ `851e9cb257ba7c66cc8c14214a31c44d2f1e954e` with `tree-sitter-cli@0.25.8`. The generated parser carries local SQLite grammar additions for `PRAGMA`, SQLite `AUTOINCREMENT`, `CREATE TRIGGER ... BEGIN ... END`, and SQLite `IF NOT EXISTS` migration forms. The existing vendored scanner is preserved because it carries this repository's deserialize cleanup.
- **tsx (refreshed 2026-09-20):** regenerated from [tree-sitter/tree-sitter-typescript](https://github.com/tree-sitter/tree-sitter-typescript) @ `75b3874edb2d` with its pinned [tree-sitter-javascript](https://github.com/tree-sitter/tree-sitter-javascript) dependency @ `3a837b6f3658` (v0.23.1) and `tree-sitter-cli@0.24.4`. The generated parser carries one local JSX token, `jsx_text_ampersand`, so ordinary raw `&` text is accepted without changing HTML entity parsing.

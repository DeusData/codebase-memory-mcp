...
- **vb_dotnet** (added 2026-07-19): vendored from [CodeAnt-AI/tree-sitter-vb-dotnet](https://github.com/CodeAnt-AI/tree-sitter-vb-dotnet) @ `cfca210ce8fd` — MIT (declared in package.json/tree-sitter.json; upstream ships no LICENSE file, so the vendored LICENSE carries the standard MIT text for the declared license, per the `move`/`zsh` convention), ABI 15, no external scanner. **NOT byte-for-byte upstream:** `parser.c` is regenerated (tree-sitter-cli 0.25.6) from a locally patched `grammar.js` — upstream at the pinned commit cannot parse several core VB.NET forms (next-line/colon `Inherits`/`Implements` clauses, `Handles` clauses, `AddHandler`/`RemoveHandler`/`RaiseEvent` statements, `As New T()` initializers, `#Region` between members, nested types, attribute `_`-continuation before declarations). The full local diff is `vb_dotnet/grammar.js.patch` (the patched `grammar.js` is vendored alongside it); on re-vendor, re-apply the patch (or confirm upstream merged the fixes) and regenerate with `--abi` ≤ 15. Patches proposed upstream: [CodeAnt-AI/tree-sitter-vb-dotnet#8](https://github.com/CodeAnt-AI/tree-sitter-vb-dotnet/pull/8).
...
| vb_dotnet | 15 | CodeAnt-AI/tree-sitter-vb-dotnet | `cfca210ce8fd` | vendor-maintained (patched) | ✅ |

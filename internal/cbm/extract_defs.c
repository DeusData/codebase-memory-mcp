...
static void extract_class_methods(CBMExtractCtx *ctx, TSNode class_node, const char *class_qn,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (ctx->language == CBM_LANG_GDSCRIPT && strcmp(ts_node_type(child), "signal_statement") == 0) {
...
static const char *compute_class_qn(CBMExtractCtx *ctx, TSNode node, const char *saved_enclosing) {
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (cname && cname[0]) {
            ...
            if (ctx->language == CBM_LANG_GDSCRIPT) {
                const char *anchor_qn = cbm_gdscript_anchor_qn(ctx->arena, ctx->root, ctx->source,
                                                               ctx->project, ctx->rel_path);
                if (anchor_qn && anchor_qn[0]) {
                    return cbm_arena_sprintf(ctx->arena, "%s.%s", anchor_qn, cname);
                }
            }

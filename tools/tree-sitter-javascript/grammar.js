/*
 * cbm self-maintained fork of tree-sitter-javascript (JSX lone-ampersand fix, #1736)
 *
 * Copyright (c) 2014 Max Brunsfeld            (tree-sitter-javascript, MIT)
 * Copyright (c) 2026 DeusData                 (the "cbm fork (#1736)" hunks below)
 *
 ...
 *
 * Fork provenance (pinned; the commit this repo vendored before the fork):
 *   tree-sitter/tree-sitter-javascript @ 58404d8cf191d69f2674a8fd507bd5776f46cb11
 *     grammar.js      -> this file = upstream + this header + patches/jsx-lone-ampersand.patch
 *     src/scanner.c   -> src/scanner.c (byte-identical)
 *
 * The patch (and nothing else) differs from upstream: upstream leaves a lone '&'
 * that does not begin a complete html_character_reference unmatched inside JSX
 * strings and JSX text (`href="?a=1&b=2"`, `<p>Tom &Jerry</p>`, `a & b`), which
 * yields an ERROR node. Node types are unchanged: the lone '&' is aliased to the
 * existing string_fragment / jsx_text nodes. The same patch is applied to the
 * javascript base of the tsx fork (tools/tree-sitter-tsx/javascript-grammar.js).
 * On an upstream bump: re-fetch the pinned sources, re-apply the patch, regenerate.
 *
 * Regenerate (ABI 15 -- the vendored runtime ceiling; NEVER use a CLI >= 0.26,
 * which emits ABI 16):
 *   cd tools/tree-sitter-javascript
 *   npx tree-sitter-cli@0.25.10 generate
 *   npx tree-sitter-cli@0.25.10 test
 *   cp src/parser.c ../../internal/cbm/vendored/grammars/javascript/parser.c
 */

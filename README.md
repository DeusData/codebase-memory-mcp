...
| Codex CLI | Detected | `$CODEX_HOME/config.toml` | Skill, three read-only agents; `SessionStart` + `SubagentStart` |
...
For Codex, install no longer creates global `$CODEX_HOME/AGENTS.md` guidance. When that file
already exists, install removes only the legacy `codebase-memory-mcp` managed block and preserves
all user-owned bytes; the file itself remains even when the cleanup leaves it empty.

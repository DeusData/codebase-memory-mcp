...
const I18N = (() => {
  const DICT = {
    en: {
      ...
      "provider.httpOnly": "only http/https URLs are supported",
      "provider.tooBig": "response too large",
      "provider.badJson": "server returned invalid JSON",
      "provider.empty": "empty response",
      "provider.timeout": "timed out after {ms} ms",
      "provider.requestError": "request failed",
      "graphcore.engineMissing": "the memory engine is not built — prepare the build from the warning above",
      "graphcore.startFailed": "could not start the 3D graph: {msg}",
      "graphcore.noUi": "the engine was built without the 3D UI — rebuild with scripts/build.sh --with-ui",
      "graphcore.exitedOnStart": "the 3D graph process exited during startup",
      "graphcore.noResponse": "the 3D graph did not respond within 6 seconds",
      "proj.stepName": "name",
      "proj.badName": "invalid project name",
      "proj.stepParent": "parent folder",
      "proj.parentMissing": "folder does not exist",
      "proj.alreadyExists": "a folder with this name already exists",
      "proj.stepDisciplines": "fablize disciplines",
      "proj.disciplinesFailed": "project created, but the fablize disciplines were not applied — run manually: bash {installer} \"{target}\"",
      "proj.installerMissing": "installer not found: {installer}",
      "proj.noDisciplines": "project created without fablize disciplines (installer not found in {dir})",
      "proj.initCommit": "MindForge: project init",
      "proj.stepFirstCommit": "first commit",
      "proj.firstCommitFailed": "project created, but the first commit failed — stories will not start; run manually: git -C \"{target}\" commit --allow-empty -m init",
    ...
    ru: {
      ...
      "provider.httpOnly": "поддерживаются только http/https адреса",
      "provider.tooBig": "ответ слишком большой",
      "provider.badJson": "сервер вернул некорректный JSON",
      "provider.empty": "пустой ответ",
      "provider.timeout": "превышено время ожидания ({ms} мс)",
      "provider.requestError": "ошибка запроса",
      "graphcore.engineMissing": "движок памяти не собран — подготовьте сборку в предупреждении сверху",
      "graphcore.startFailed": "не удалось запустить 3D-граф: {msg}",
      "graphcore.noUi": "движок собран без 3D UI — пересоберите через scripts/build.sh --with-ui",
      "graphcore.exitedOnStart": "процесс 3D-графа завершился при запуске",
      "graphcore.noResponse": "3D-граф не ответил за 6 секунд",
      "proj.stepName": "имя",
      "proj.badName": "недопустимое имя проекта",
      "proj.stepParent": "родительская папка",
      "proj.parentMissing": "папка не существует",
      "proj.alreadyExists": "такая папка уже существует",
      "proj.stepDisciplines": "дисциплины fablize",
      "proj.disciplinesFailed": "проект создан, но дисциплины fablize не применились — выполните вручную: bash {installer} \"{target}\"",
      "proj.installerMissing": "installer не найден: {installer}",
      "proj.noDisciplines": "проект создан без дисциплин fablize (installer не найден в {dir})",
      "proj.initCommit": "MindForge: инициализация проекта",
      "proj.stepFirstCommit": "первый коммит",
      "proj.firstCommitFailed": "проект создан, но первый коммит не удался — истории не запустятся, выполните вручную: git -C \"{target}\" commit --allow-empty -m init",

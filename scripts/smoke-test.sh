...
if [ "$DL_OS" != "windows" ] && [ -f "$REPO_ROOT/install.sh" ]; then
  ...
  # -NoProfile and the TEMP/TMP redirection come from upstream; the failure check
  # is kept rather than upstream's `|| true`, which discarded the installer's exit
  # status and let a failed install pass the phase.
  if ! HOME="$PS1_TEST_HOME" CBM_DOWNLOAD_URL="$WIN_URL" CBM_ARCH="$DL_ARCH" \
    powershell.exe -NoProfile -ExecutionPolicy ByPass -Command \
      '$env:TEMP=$args[0]; $env:TMP=$args[0]; & $args[1] $args[2]' \
      "$WIN_HOME" "$WIN_SCRIPT" "--dir=$WIN_DIR" 2>&1; then

...
function summaryFileToPlanId(summaryFile) {
  if (summaryFile === 'SUMMARY.md') return '';

  const directPlanId = summaryFile.replace('-SUMMARY.md', '');
  if (/^\d+-\d+$/.test(directPlanId)) {
    return directPlanId;
  }

  const verboseMatch = summaryFile.match(/^(\d+)-.+-(\d+)-SUMMARY\.md$/);
  if (verboseMatch) {
    return `${verboseMatch[1]}-${verboseMatch[2]}`;
  }

  return summaryFile.replace('SUMMARY.md', '');
}
...
function cmdPhasePlanIndex(cwd, phase, raw) {
  ...
  const completedPlanIds = new Set(
    summaryFiles.map(summaryFileToPlanId).filter(Boolean)
  );
  ...
  if (fs.existsSync(roadmapPath)) {
    withPlanningLock(cwd, () => {
      ...
      for (const summaryFile of phaseInfo.summaries) {
        const planId = summaryFileToPlanId(summaryFile);
        if (!planId) continue;

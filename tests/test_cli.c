...
TEST(cli_install_plan_receipt_no_mutation_issue388) {
    ...
    if (!missing) {
        ...
        if (!doc) {
            missing = "a valid JSON document";
        } else {
            yyjson_val *cleanups =
                yyjson_obj_get(yyjson_doc_get_root(doc), "cleanup_actions_planned");
            if (!cleanups || !yyjson_is_arr(cleanups) || yyjson_arr_size(cleanups) != 0U) {
                missing = "an empty cleanup_actions_planned array";
            }
        }
    }
    if (doc) {

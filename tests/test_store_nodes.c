...
TEST(store_open_path_existing_requires_existing_writable_db) {
    char *tmp_dir = th_mktempdir("cbm_store_existing");
    ASSERT_NOT_NULL(tmp_dir);
    char path[CBM_PATH_MAX];
    int n = snprintf(path, sizeof(path), "%s/store.db", tmp_dir);
    ASSERT_GT(n, 0);
    ASSERT_LT(n, (int)sizeof(path));

    ASSERT_NULL(cbm_store_open_path_existing(path));
    ASSERT_FALSE(cbm_file_exists(path));

    cbm_store_t *created = cbm_store_open_path(path);
    ASSERT_NOT_NULL(created);
    cbm_store_close(created);

    cbm_store_t *existing = cbm_store_open_path_existing(path);
    ASSERT_NOT_NULL(existing);
    ASSERT_EQ(cbm_store_upsert_project(existing, "existing", tmp_dir), CBM_STORE_OK);
    cbm_store_close(existing);
...
TEST(store_lsp_surface_round_trip) {
    ...
    const char *selected_paths[] = {"pkg/b.go", "pkg/missing.go", "pkg/a.go"};
    ASSERT_EQ(cbm_store_get_lsp_surfaces_by_paths(s, "test", selected_paths, 3, &rows, &count),
              CBM_STORE_OK);
    ASSERT_EQ(count, 2);
    ASSERT_STR_EQ(rows[0].rel_path, "pkg/b.go");
    ASSERT_STR_EQ(rows[1].rel_path, "pkg/a.go");
    cbm_store_free_lsp_surfaces(rows, count);
...
TEST(store_compact_next_overlay_generation_returns_not_found_without_ready_overlay) {
    ...
    int compacted = SENTINEL;
    ASSERT_EQ(cbm_store_compact_ready_overlay_generations(
                  s, "test", CBM_STORE_COMPACT_ALL_GENERATIONS, &compacted),
              CBM_STORE_OK);
...
TEST(store_overlay_additions_keep_canonical_file_rows_visible) {
    enum { BASE_GENERATION = 1 };
    cbm_store_t *s = cbm_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(cbm_store_upsert_project(s, "test", "/tmp/test"), CBM_STORE_OK);
...
TEST(store_compact_ready_overlay_generations_respects_batch_limit) {
    ...
    ASSERT_EQ(cbm_store_reserve_overlay_generation(s, "test", BASE_GENERATION,
                                                   &first_overlay),
              CBM_STORE_OK);
    cbm_store_file_delta_t delete_main = {.project = "test",
                                          .rel_path = "main.go",
                                          .generation = BASE_GENERATION,
                                          .derived_view_name =
    ...
    ASSERT_EQ(cbm_store_publish_overlay_file_delta(s, &delete_main, first_overlay),
              CBM_STORE_OK);
    ...
    ASSERT_EQ(cbm_store_reserve_overlay_generation(s, "test", BASE_GENERATION,
                                                   &second_overlay),
              CBM_STORE_OK);
    cbm_store_file_delta_t delete_helper = {.project = "test",
                                            .rel_path = "helper.go",
                                            .generation = BASE_GENERATION,
                                            .derived_view_name =
                                                CBM_STORE_DERIVED_VIEW_NODES_FTS,
                                            .derived_status =
                                                CBM_STORE_DERIVED_STATUS_COMPLETE};
    ASSERT_EQ(cbm_store_publish_overlay_file_delta(s, &delete_helper, second_overlay),
              CBM_STORE_OK);

    int compacted = -1;
    ASSERT_EQ(cbm_store_compact_ready_overlay_generations(
                  s, "test", COMPACT_ONE_GENERATION, &compacted),
              CBM_STORE_OK);
    ASSERT_EQ(compacted, 1);
    ASSERT_EQ(store_count_overlay_generation_row(s, "test", first_overlay,
                                                 BASE_GENERATION,
                                                 CBM_STORE_OVERLAY_STATUS_READY),
              0);
    ASSERT_EQ(store_count_overlay_generation_row(s, "test", second_overlay,
                                                 BASE_GENERATION,
                                                 CBM_STORE_OVERLAY_STATUS_READY),
              1);
    ASSERT_EQ(store_node_qn_exists(s, "test", "test.main.Old"), 0);
...
TEST(store_search_overlay_view_dedupes_multi_owner_active_edges) {
    ...
    ASSERT_STR_EQ(schema.rel_patterns[0].source_label, "Function");
    ASSERT_STR_EQ(schema.rel_patterns[0].edge_type, "CALLS");
    ASSERT_STR_EQ(schema.rel_patterns[0].target_label, "Function");
    ASSERT_EQ(schema.rel_patterns[0].observed_count, 1);
...
TEST(store_schema_counts_overlay_view_uses_active_nodes_and_edges) {
    ...
    ASSERT_STR_EQ(schema.rel_patterns[0].source_label, "Function");
    ASSERT_STR_EQ(schema.rel_patterns[0].edge_type, "CALLS");
    ASSERT_STR_EQ(schema.rel_patterns[0].target_label, "Class");
    ASSERT_EQ(schema.rel_patterns[0].observed_count, 1);
    cbm_store_schema_free(&schema);
    ...
    ASSERT_EQ(schema.rel_pattern_count, 1);
    ASSERT_STR_EQ(schema.rel_patterns[0].source_label, "Route");
    ASSERT_STR_EQ(schema.rel_patterns[0].edge_type, "HANDLES");
    ASSERT_STR_EQ(schema.rel_patterns[0].target_label, "Class");
    ASSERT_EQ(schema.rel_patterns[0].observed_count, 1);
    cbm_store_schema_free(&schema);
...
SUITE(store_nodes) {
    ...
    RUN_TEST(store_open_path_existing_requires_existing_writable_db);
    ...
    RUN_TEST(store_overlay_additions_keep_canonical_file_rows_visible);

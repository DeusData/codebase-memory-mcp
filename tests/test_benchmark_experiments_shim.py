...
CAMPAIGN_SCRIPT = SCRIPTS_ROOT / "run-benchmark-campaign.py"
...
CAMPAIGN = _load(CAMPAIGN_SCRIPT, "run_benchmark_campaign_shim_direct")
...
class BenchmarkExperimentsEntryPointTest(unittest.TestCase):
    ...
    def test_campaign_shim_resolves_and_re_exports_the_experiments_implementation(self) -> None:
        # The shim loads run-benchmark-experiments.py by path and republishes its
        # public names, so callers that import run-benchmark-campaign.py directly
        # (tests/test_benchmark_campaign.py, scripts/autotune.py, scripts/
        # summarize-benchmark-results.py) keep working without modification. Each
        ...
        self.assertEqual(Path(CAMPAIGN._impl.__file__).resolve(), EXPERIMENTS_SCRIPT.resolve())
        self.assertTrue(hasattr(CAMPAIGN, "main"))
        self.assertTrue(hasattr(CAMPAIGN, "parse_arguments"))
        self.assertTrue(hasattr(CAMPAIGN, "build_automatic_spec"))
        self.assertEqual(CAMPAIGN.DEFAULT_CANDIDATE_REFS, EXPERIMENTS.DEFAULT_CANDIDATE_REFS)
        self.assertEqual(
            CAMPAIGN.parse_arguments(["--experiment-root", "r", "--plan", "p.json"]).campaign_root,
            EXPERIMENTS.parse_arguments(["--experiment-root", "r", "--plan", "p.json"]).campaign_root,
        )

    def test_experiment_root_flag_is_an_alias_for_campaign_root_on_both_entry_points(self) -> None:
        for module in (EXPERIMENTS, CAMPAIGN):
            ...
            self.assertEqual(via_alias.campaign_root, Path("runs-here"))
            self.assertEqual(via_alias.campaign_root, via_legacy.campaign_root)

    def test_allow_temporary_experiment_root_flag_is_an_alias(self) -> None:
        for module in (EXPERIMENTS, CAMPAIGN):
            via_alias = module.parse_arguments(
                ["--allow-temporary-experiment-root", "--plan", "p.json", "--campaign-root", "r"]
            )
            via_legacy = module.parse_arguments(
                ["--allow-temporary-campaign-root", "--plan", "p.json", "--campaign-root", "r"]
            )
            self.assertTrue(via_alias.allow_temporary_campaign_root)
            self.assertTrue(via_legacy.allow_temporary_campaign_root)

    def test_legacy_campaign_root_flag_still_works_without_any_alias(self) -> None:
        args = CAMPAIGN.parse_arguments(["--campaign-root", "legacy-results", "--plan", "p.json"])
        self.assertEqual(args.campaign_root, Path("legacy-results"))

    def test_plan_invocation_is_byte_identical_between_old_and_new_script_names(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            campaign_root = root / "results"
            campaign_root.mkdir()
            ...
            # identical without touching the filesystem beyond campaign_root.
            legacy = subprocess.run(
                [sys.executable, str(CAMPAIGN_SCRIPT), "--plan", str(plan_path),
                 "--campaign-root", str(campaign_root), "--allow-temporary-campaign-root",
                 "--audit-only"],
                capture_output=True, text=True,
            )
            new = subprocess.run(
                [sys.executable, str(EXPERIMENTS_SCRIPT), "--plan", str(plan_path),
                 "--experiment-root", str(campaign_root), "--allow-temporary-experiment-root",
                 "--audit-only"],
                capture_output=True, text=True,
            )
...
class CandidateRefOverrideTest(unittest.TestCase):
    ...
    def test_candidate_ref_cli_flag_populates_args_and_rejects_unknown_label(self) -> None:
        args = EXPERIMENTS.parse_arguments(["--quick", "--candidate-ref", "latest=HEAD~1"])
    ...
    def test_candidate_ref_flag_rejects_use_without_automatic_preset(self) -> None:
        with self.assertRaises(SystemExit):
            EXPERIMENTS.parse_arguments(
                ["--plan", "p.json", "--campaign-root", "r", "--candidate-ref", "latest=HEAD~1"]
            )
    ...
    def test_resolve_default_candidate_ref_only_touches_upstream_main(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                EXPERIMENTS.resolve_default_candidate_ref(repo, "pre-today-major", "some-tag"),
                "some-tag",
            )

    def test_resolve_default_candidate_ref_falls_back_from_upstream_to_origin_main(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir)
            _git("init", "-q", str(repo), cwd=Path.cwd())
            ...
            resolved = EXPERIMENTS.resolve_default_candidate_ref(repo, "upstream-main", "upstream/main")

            self.assertEqual(resolved, "origin/main")

    def test_resolve_default_candidate_ref_returns_original_ref_when_nothing_resolves(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir)
            _git("init", "-q", str(repo), cwd=Path.cwd())
            _git("config", "user.email", "test@example.invalid", cwd=repo)
            ...
            if current_branch in {"upstream/main", "origin/main", "main"}:
                self.skipTest("git init default branch collides with fallback ref under test")

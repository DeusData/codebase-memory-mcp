...
/* The reporter's third control: blanks BEFORE a real newline were always clean
 * and must stay that way, pinning the pair-trigger from both sides. */
TEST(dockerfile_trailing_blank_then_newline_still_clean_issue1746) {
    ...
    cbm_free_result(r);
    if (flagged) {
        FAIL("a terminated Dockerfile must not be parse_partial because of trailing blanks");
    }
    PASS();
}
...
SUITE(parse_coverage) {
    ...
    RUN_TEST(dockerfile_trailing_blank_then_newline_still_clean_issue1746);

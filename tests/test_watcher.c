...
TEST(watcher_prunes_sustained_missing_root) {
    ...
    /* Sustained absence is minutes-scale: keep the project before threshold. */
    for (int i = 1; i < 35; i++) {
        cbm_watcher_touch(w, "stale-project");
        cbm_watcher_poll_once(w);
    }
    ...
    /* Threshold reached: prune watcher state and cached DB files. */
    cbm_watcher_touch(w, "stale-project");
    cbm_watcher_poll_once(w);
    ASSERT_EQ(cbm_watcher_watch_count(w), 0);
...
TEST(watcher_does_not_prune_inaccessible_root) {
#ifdef _WIN32
    PASS();
#else
    char parentdir[256];
    snprintf(parentdir, sizeof(parentdir), "/tmp/cbm_watcher_inaccessible_parent_XXXXXX");
    if (!cbm_mkdtemp(parentdir)) {
        FAIL("cbm_mkdtemp parent failed");
    }

    char rootdir[512];
    snprintf(rootdir, sizeof(rootdir), "%s/root", parentdir);
    if (mkdir(rootdir, 0700) != 0) {
        th_rmtree(parentdir);
        FAIL("mkdir root failed");
    }

    char cachedir[256];
    snprintf(cachedir, sizeof(cachedir), "/tmp/cbm_watcher_inaccessible_cache_XXXXXX");
    if (!cbm_mkdtemp(cachedir)) {
        th_rmtree(parentdir);
        FAIL("cbm_mkdtemp cache failed");
    }
    ...
    char db_path[512];
    snprintf(db_path, sizeof(db_path), "%s/inaccessible-project.db", cachedir);
    th_write_file(db_path, "db\n");

    cbm_store_t *store = cbm_store_open_memory();
    cbm_watcher_t *w = cbm_watcher_new(store, index_callback, NULL);
    cbm_watcher_watch(w, "inaccessible-project", rootdir);
    cbm_watcher_poll_once(w);

    if (chmod(parentdir, 0000) != 0) {
        cbm_watcher_free(w);
        cbm_store_close(store);
        if (had_cache_dir) {
            cbm_setenv("CBM_CACHE_DIR", saved_cache_dir, 1);
        ...
        th_rmtree(cachedir);
        th_rmtree(parentdir);
        FAIL("chmod parent failed");
    }

    for (int i = 0; i < 40; i++) {
        cbm_watcher_touch(w, "inaccessible-project");
        cbm_watcher_poll_once(w);
    }
    int watch_count = cbm_watcher_watch_count(w);
    int db_exists = access(db_path, F_OK);

    chmod(parentdir, 0700);
    cbm_watcher_free(w);
    cbm_store_close(store);
    if (had_cache_dir) {
        cbm_setenv("CBM_CACHE_DIR", saved_cache_dir, 1);
...
SUITE(watcher) {
    ...
    RUN_TEST(watcher_does_not_prune_inaccessible_root);

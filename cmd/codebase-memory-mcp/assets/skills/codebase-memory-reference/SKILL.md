...
| `trace_path` | BFS call chain traversal. Supports `risk_labels=true`, `compact=true`, `max_results`. |
...
| `get_architecture` | Architecture summary, clusters, routes, dependencies, and key functions |
| `manage_adr` | Read or update Architecture Decision Records |
| `ingest_traces` | Ingest OpenTelemetry traces to validate HTTP_CALLS edges |
| `index_dependencies` | Index local dependency source under `{project}.dep.{name}` |
...
query_graph(query="MATCH (f)-[:CALLS]->(g) WHERE f.qualified_name =~ '.*\\.services\\..*' RETURN f.name, count(g) AS out_degree ORDER BY out_degree DESC LIMIT 20")
...
| `compact=true` | `search_graph`, `trace_path` | Omit `name` field when redundant with `qualified_name` (~15-25% reduction) |
...
| `max_results=N` | `trace_path` | Cap BFS results per direction (default 25) |
...
3. **`trace_path` works best with exact names** — use `search_graph(name_pattern=".*Partial.*")` first to discover names.

...
static void add_response_stale_view(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    yyjson_mut_val *stale_views = yyjson_mut_obj_get(freshness, CBM_MCP_FRESHNESS_STALE_VIEWS_KEY);
...
static bool get_dirty_file_counts(cbm_store_t *store, const char *project, int *out_pending,
...
static void add_dirty_file_freshness_counts(yyjson_mut_doc *doc, yyjson_mut_val *root, int pending,
                                            int overlay_ready) {
    cbm_mcp_add_dirty_file_freshness_counts(doc, root, pending, overlay_ready, NULL);
}

static void add_dirty_file_freshness(yyjson_mut_doc *doc, yyjson_mut_val *root, cbm_store_t *store,
                                     const char *project) {
    int pending = 0;
    int overlay_ready = 0;
...
static void add_overlay_node_read_view_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    yyjson_mut_obj_add_int(doc, view, "active_file_tombstones", summary.active_file_tombstones);
    yyjson_mut_obj_add_int(doc, view, "canonical_nodes_visible", summary.canonical_nodes_visible);
...
static void add_overlay_active_node_search_freshness(
    ...
    yyjson_mut_obj_add_str(doc, freshness, CBM_MCP_FRESHNESS_READ_MODEL_KEY,
                           uses_active_edges ? CBM_MCP_FRESHNESS_READ_MODEL_OVERLAY_ACTIVE_GRAPH
...
static void add_overlay_active_schema_freshness(
    yyjson_mut_doc *doc, yyjson_mut_val *root, const cbm_store_overlay_node_view_summary_t *summary,
    bool include_properties, const char *warning) {
    if (!cbm_store_overlay_node_view_has_ready_rows(summary)) {
        return;
    }
    yyjson_mut_val *freshness = ensure_response_freshness(doc, root);
...
static bool add_overlay_active_architecture_freshness(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                                      cbm_store_t *store, const char *project,
                                                      bool include_languages,
                                                      bool include_entry_points,
                                                      bool include_routes, bool include_file_tree,
                                                      const char *warning) {
    if (!doc || !root || !store || !project || !project[0]) {
        return false;
    }
...
static void add_pipeline_exact_delta_stats(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    if (!doc || !root ||
        (stats.changed_paths < 0 && stats.affected_paths < 0 && stats.published_paths < 0)) {
        return;
    }
    ...
    if (stats.affected_paths_limit >= 0) {
        yyjson_mut_obj_add_int(doc, exact, "affected_paths_limit", stats.affected_paths_limit);
    }
...
static bool search_graph_uses_route_derived_graph(const char *label, const char *relationship) {
    return (label && strcmp(label, "Route") == 0) ||
           query_mentions_route_derived_graph(relationship);
}
...
static bool cypher_result_contains_route_label(const cbm_cypher_result_t *result) {
    ...
    for (int c = 0; c < result->col_count; c++) {
        ...
        for (int r = 0; r < result->row_count; r++) {
            if (result->rows[r] && result->rows[r][c] && strcmp(result->rows[r][c], "Route") == 0) {
                return true;
            }
        }
    }
...
static const tool_def_t TOOLS[] = {
    {"index_repository", "Index repository",
     ...
     "\"Set false to skip dependency package indexing for this call. Default follows config "
     "auto_index_deps.\"},"
     "\"auto_dep_limit\":{\"type\":\"integer\",\"minimum\":0,\"maximum\":" CBM_STRINGIFY(
         CBM_MAX_AUTO_DEP_LIMIT) ",\"description\":"
                                 "\"Dependency package cap for this call from 1 to " CBM_STRINGIFY(
                                     CBM_MAX_AUTO_DEP_LIMIT) ". Default follows config "
                                                             "auto_dep_limit; 0 means "
                                                             "unlimited.\"},"
                                                             "\"name\":{\"type\":\"string\","
                                                             "\"description\":"
                                                             "\"Override the derived project name. "
                                                             "Non-ASCII bytes are encoded and "
                                                             "unsafe path characters "
                                                             "are normalized.\"},"
                                                             "\"persistence\":{\"type\":"
                                                             "\"boolean\",\"default\":false,"
                                                             "\"description\":"
                                                             "\"Write compressed artifact to "
                                                             ".codebase-memory/graph.db.zst for "
                                                             "team sharing. "
                                                             "Teammates can bootstrap from the "
                                                             "artifact instead of full "
                                                             "re-indexing.\"}"
                                                             ",\"format\":{\"type\":\"string\","
                                                             "\"enum\":[\"toon\",\"json\"],"
                                                             "\"default\":\"toon\","
                                                             "\"description\":\"Compact TOON by "
    ...
    {"search_graph", "Search graph",
     ...
     "Returns structured results in one call. Compact rows use in/out = selected degree across "
     "CALLS, USAGE, CALL_REFERENCE, INHERITS, and IMPLEMENTS; use trace_path for caller/callee "
     "counts. "
     ...
     "from server CWD; first use may auto-index "
     "it.\"},\"label\":{\"type\":\"string\",\"description\":\"Node label filter, "
     "for example Function, Class, Method, Route, or "
     "File.\"},\"name_pattern\":{\"type\":\"string\",\"description\":\"Regex pattern on symbol "
     ...
     "\"boolean\",\"description\":\"Include directly connected symbols for each "
     "match.\"},\"limit\":{\"type\":"
     "\"integer\",\"description\":\"Max results per page (configurable via search_limit config "
     "key). "
    ...
    {"query_graph", "Query graph",
     ...
     "graph or the best-effort graph of files not fully "
     "indexed.\"},\"format\":{\"type\":\"string\","
    ...
    {"trace_path", "Trace path",
     ...
     "\"},\"qualified_name\":{\"type\":\"string\",\"description\":\"Exact qualified name from "
     "search "
     ...
     "use the MCP server project derived from server CWD; first use may auto-index "
     "it.\"},\"direction\":{\"type\":\"string\",\"enum\":[\"inbound\",\"outbound\","
     ...
     "a prior trace_path response. Keep every other traversal argument "
     "identical.\"},\"compact\":{\"type\":\"boolean\","
     "\"default\":true,\"description\":"
     "\"Per-call override for the compact config key (true by default). Omit name when it equals "
     "qualified_name's last segment (e.g. \\\"main\\\" in \\\"pkg.main\\\"). Reduces token "
     "count.\"},"
     ...
     "\"description\":\"Include test/spec file nodes in trace results and mark them with "
     "is_test.\"},"
     "\"risk_labels\":{\"type\":\"boolean\",\"default\":false,"
     "\"description\":\"Annotate traced nodes with CRITICAL/HIGH/MEDIUM/LOW risk by hop "
     "distance.\"},"
     "\"parameter_name\":{\"type\":\"string\",\"description\":\"Accepted for upstream "
     "compatibility; "
     "reserved for future parameter-level data-flow narrowing."
     "\"},\"include_evidence\":{\"type\":\"boolean\",\"default\":false,"
     "\"description\":\"Add each hop's deterministic shortest-path predecessor resolver "
     "strategy (lsp, language_rule, heuristic, or unresolved) and confidence. Off by default "
     "because it adds response columns.\"},"
     "\"format\":{\"type\":\"string\",\"enum\":[\"toon\",\"json\"],\"default\":\"toon\","
    ...
    {"get_code_snippet", "Get code snippet",
     "Get source code for a specific function, class, or symbol by qualified name. Prefer this "
     "over "
     ...
     "project derived from server "
     "CWD.\"},\"auto_resolve\":{\"type\":\"boolean\",\"default\":false,\"description\":"
     ...
     "response includes total_lines and signature for "
     "context.\"},\"mode\":{\"type\":\"string\",\"enum\":[\"full\",\"signature\","
    ...
    {"get_architecture", "Get architecture",
     ...
     "\"Indexed project name or repository directory. Omit to use the MCP server project derived "
     "from server CWD; first use may auto-index "
     "it.\"},\"path\":{\"type\":\"string\",\"description\":"
    ...
    {"search_code", "Search code",
     ...
     "server project after it has been "
     "indexed.\"},\"file_pattern\":{\"type\":\"string\",\"description\":\"Glob for grep "
     "--include; use this to reduce traversal (e.g. "
     "*.go).\"},\"path_filter\":{\"type\":\"string\",\"description\":\"Regex "
     ...
     "\"description\":\"Number of surrounding lines to include around each match (like grep -C). "
     "Default 0.\"},"
     "\"mode\":{\"type\":\"string\",\"enum\":[\"compact\",\"full\",\"files\"],\"default\":"
     "\"compact\","
    ...
    {"ingest_traces", "Ingest traces",
     ...
     "\"string\",\"description\":\"Indexed project name receiving the trace "
     "data.\"}},\"required\":[\"traces\",\"project\"]}"},
    ...
    {"index_dependencies", "Index dependencies",
     ...
     "{\"type\":\"object\",\"properties\":{"
     "\"project\":{\"type\":\"string\",\"description\":\"Existing indexed project to add deps "
     "to\"},"
...
static const tool_def_t STREAMLINED_TOOLS[] = {
    {"get_code", "Get code",
     ...
     "Prefer this over reading entire files. Use mode=signature for API lookup without source "
     "body. "
     ...
     "\"description\":\"Per-call override for the compact config key (true by default). Omit name "
     "when it equals last segment of qualified_name.\"}"
...
static void emit_tool(yyjson_mut_doc *doc, yyjson_mut_val *tools, const tool_def_t *tool_def,
    ...
    yyjson_mut_obj_add_str(doc, tool, "title", tool_def->title ? tool_def->title : tool_def->name);
...
static bool is_streamlined_default_tool(const char *name) {
    ...
    return name && (strcmp(name, "search_graph") == 0 || strcmp(name, "query_graph") == 0 ||
...
static bool mcp_tool_allowed(cbm_mcp_tool_profile_t profile, const char *name) {
    ...
    const char *const *allowed = profile == CBM_MCP_TOOL_PROFILE_ANALYSIS ? analysis_tools
                                 : profile == CBM_MCP_TOOL_PROFILE_SCOUT  ? scout_tools
                                                                          : NULL;
...
static bool mcp_client_requires_static_tool_catalog(const char *params_json) {
    ...
    yyjson_val *root = yyjson_doc_get_root(doc);
    yyjson_val *client_info =
        root && yyjson_is_obj(root) ? yyjson_obj_get(root, "clientInfo") : NULL;
    yyjson_val *name =
        client_info && yyjson_is_obj(client_info) ? yyjson_obj_get(client_info, "name") : NULL;
...
static const char *mcp_prompt_string_argument(yyjson_val *arguments, const char *name) {
    yyjson_val *value =
        arguments && yyjson_is_obj(arguments) ? yyjson_obj_get(arguments, name) : NULL;
...
static char *cbm_mcp_prompt_get(const char *params_json, int *error_code,
    ...
    *error_message = NULL;
    ...
    static const char EXPLORE_TEMPLATE[] =
        ...
        "and trace_path(direction=\"both\") for callers and callees. Check coverage and "
        "pagination; "
...
static bool ends_with_segment(const char *qn, const char *name) {
    ...
    if (name_len > qn_len)
        return false;
    if (name_len == qn_len)
        return strcmp(qn, name) == 0;
    char sep = qn[qn_len - name_len - 1];
    return (sep == '.' || sep == ':' || sep == '/') && strcmp(qn + qn_len - name_len, name) == 0;
}
...
struct cbm_mcp_server {
     ...
     * diagnostic pointer for index_repository only, published and cleared under
     * active_request_lock so the handoff cannot race a concurrent bind. */
...
static bool cbm_mcp_tool_mode_is_classic(cbm_mcp_server_t *srv) {
    ...
    const char *tool_mode =
        cbm_safe_getenv("CBM_TOOL_MODE", tool_mode_buf, sizeof(tool_mode_buf), NULL);
...
static cbm_mcp_output_format_t cbm_mcp_response_format(cbm_mcp_server_t *srv, const char *args) {
    char *override = cbm_mcp_get_string_arg(args, "format");
    const char *value = override;
    if (!value) {
        value =
            cbm_config_get_effective(srv ? srv->config : NULL, CBM_CONFIG_DEFAULT_RESPONSE_FORMAT,
...
static int cbm_mcp_config_int_clamped(cbm_mcp_server_t *srv, const char *key, int default_val,
    ...
    int value =
        srv && srv->config ? cbm_config_get_int(srv->config, key, default_val) : default_val;
    if (value < min_val) {
        value = min_val;
    }
    if (value > max_val) {
...
static int cbm_mcp_get_positive_int_arg(const char *args_json, const char *key, int default_val,
                                        int fallback_val) {
...
static int cbm_mcp_store_idle_timeout_s(cbm_mcp_server_t *srv) {
    return cbm_mcp_config_int_clamped(srv, CBM_CONFIG_STORE_IDLE_TIMEOUT_S,
                                      CBM_MCP_DEFAULT_STORE_IDLE_TIMEOUT_S, 1, CBM_SZ_64K);
}
...
static bool cbm_mcp_incremental_metadata_enabled(cbm_mcp_server_t *srv) {
    const char *policy = srv && srv->config
...
static bool cbm_mcp_overlay_compaction_after_publish(cbm_mcp_server_t *srv) {
    const char *policy = srv && srv->config
                             ...
                             : CBM_CONFIG_OVERLAY_COMPACTION_POLICY_MANUAL;
    return policy && strcmp(policy, CBM_CONFIG_OVERLAY_COMPACTION_POLICY_AFTER_PUBLISH) == 0;
}
...
static int cbm_mcp_effective_auto_dep_limit(cbm_mcp_server_t *srv, const char *args_json) {
    bool enabled = cbm_config_get_bool(srv ? srv->config : NULL, CBM_CONFIG_AUTO_INDEX_DEPS,
...
static int cbm_mcp_auto_index_deps(cbm_mcp_server_t *srv, const char *project,
                                   ...
                                   int effective_dep_limit, cbm_dep_auto_index_stats_t *out_stats,
                                   int *out_rc) {
    if (out_rc) {
        *out_rc = CBM_STORE_OK;
    }
    int deps_reindexed = cbm_dep_auto_index_effective_with_stats(
        project, root_path, store, effective_dep_limit, srv ? srv->config : NULL, out_stats);
    if (deps_reindexed > 0 && cbm_mcp_incremental_metadata_enabled(srv)) {
        int owner_rc =
            cbm_store_rebuild_file_delta_owners(store, project, CBM_PIPELINE_FILE_DELTA_GENERATION);
        if (owner_rc != CBM_STORE_OK) {
            char rc_buf[CBM_SZ_32];
            snprintf(rc_buf, sizeof(rc_buf), "%d", owner_rc);
            cbm_log_error("index_repository.err", "phase", "rebuild_file_delta_owners_after_deps",
                          "rc", rc_buf);
            if (out_rc) {
                *out_rc = owner_rc;
            }
...
/* The one owner of everything a successful publication must still do after the
 * pipeline run: dependency source indexing under the effective limit, rank
 * refresh under the configured policy, and the index-published notification
 * that invalidates cached stores. Every publication path routes through this
 * so explicit index_repository, background auto-index, watcher re-index, and
 * the daemon's shared job cannot drift apart. args_json may be NULL when the
 * path has no per-call overrides; out_stats and out_deps_reindexed are
 * optional. Returns CBM_STORE_OK, or the failing status when dependency
 * indexing could not refresh file-delta owner metadata. */
static int cbm_mcp_finish_index_publication(
    cbm_mcp_server_t *srv, const char *project, const char *root_path, cbm_store_t *store,
    const char *args_json, bool graph_changed, cbm_pipeline_publish_kind_t publish_kind,
    bool incremental_fallback, cbm_dep_auto_index_stats_t *out_stats, int *out_deps_reindexed) {
    int dep_owner_rc = CBM_STORE_OK;
    int effective_dep_limit = cbm_mcp_effective_auto_dep_limit(srv, args_json);
    int deps_reindexed = cbm_mcp_auto_index_deps(srv, project, root_path, store,
                                                 effective_dep_limit, out_stats, &dep_owner_rc);
    (void)cbm_pagerank_refresh_after_publish(
        store, project, srv ? srv->config : NULL, graph_changed, deps_reindexed,
        cbm_rank_refresh_publish_from_pipeline(publish_kind, incremental_fallback));
    /* Freshness contract: every publication path notifies, so a supervised
     * worker and an in-process publish look identical to a session. */
    cbm_mcp_server_notify_index_published(srv);
    if (out_deps_reindexed) {
        *out_deps_reindexed = deps_reindexed;
    }
    return dep_owner_rc;
}

static void cbm_mcp_add_dependency_auto_index_stats(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    yyjson_mut_obj_add_int(doc, detail, "package_limit",
    ...
    yyjson_mut_obj_add_int(doc, detail, "candidates_observed", stats->candidates_observed);
    yyjson_mut_obj_add_int(doc, detail, "packages_selected", stats->packages_selected);
    yyjson_mut_obj_add_int(doc, detail, "packages_current", stats->packages_current);
    yyjson_mut_obj_add_int(doc, detail, "packages_reindexed", stats->packages_reindexed);
    yyjson_mut_obj_add_int(doc, detail, "packages_failed", stats->packages_failed);
    yyjson_mut_obj_add_bool(doc, detail, "package_limit_hit", stats->package_limit_hit);
    yyjson_mut_obj_add_int(doc, detail, "dependency_file_limit", stats->dependency_file_limit);
    ...
    if (stats->package_limit_hit && stats->packages_skipped_file_limit > 0) {
        ...
        yyjson_mut_obj_add_str(doc, detail, "hint",
...
static void cbm_mcp_refresh_auto_indexed_store(cbm_mcp_server_t *srv, cbm_store_t *resolved_store,
    ...
    if (writable_store) {
        ...
        (void)cbm_mcp_auto_index_deps(srv, project, root_path, writable_store, effective_dep_limit,
                                      NULL, NULL);
...
static bool schema_property_in_base(const char *property, const char *const *base, int base_count);
...
static void schema_description_append_properties(cbm_sb_t *sb, char *const *properties,
                                                 int property_count, const char *const *base,
                                                 int base_count) {
    bool first = true;
...
static void schema_description_append_list(cbm_sb_t *sb, const char *const *items,
                                           size_t item_count, const char *separator) {
    for (size_t i = 0; i < item_count; i++) {
        cbm_sb_append(sb, i ? separator : "");
        cbm_sb_append(sb, items[i]);
    }
}

/* Generate the invariant language prefix from the Cypher module's typed
 * registry. Project graph vocabulary is appended separately below. Runtime is
 * O(C) for fixed capability bytes C, with no graph access and only the shared
 * output builder's O(C) retained memory. */
static void schema_description_append_cypher_capabilities(cbm_sb_t *sb) {
    const cbm_cypher_capability_schema_t *schema = cbm_cypher_capability_schema();
    cbm_sb_append(sb, " Stable Cypher capability schema ");
    cbm_sb_append(sb, schema->schema_id);
    cbm_sb_append(sb, "/v");
    schema_description_append_int(sb, (int)schema->version);
    cbm_sb_append(sb, ". Supported read-only Cypher subset. Clauses: ");
    schema_description_append_list(sb, schema->clauses, schema->clause_count, "/");
    cbm_sb_append(sb, ". Patterns: ");
    schema_description_append_list(sb, schema->patterns, schema->pattern_count, "; ");
    cbm_sb_append(sb, ". Predicates: ");
    schema_description_append_list(sb, schema->predicates, schema->predicate_count, "; ");
    cbm_sb_append(sb, ". Aggregates: ");
    schema_description_append_list(sb, schema->aggregate_functions,
                                   schema->aggregate_function_count, "/");
    cbm_sb_append(sb, " with DISTINCT aggregate arguments. Scalar functions: ");
    schema_description_append_list(sb, schema->keyword_scalar_functions,
                                   schema->keyword_scalar_function_count, "/");
    if (schema->keyword_scalar_function_count && schema->named_scalar_function_count) {
        cbm_sb_append(sb, "/");
    }
    schema_description_append_list(sb, schema->named_scalar_functions,
                                   schema->named_scalar_function_count, "/");
    if ((schema->keyword_scalar_function_count || schema->named_scalar_function_count) &&
        schema->multi_argument_function_count) {
        cbm_sb_append(sb, "/");
    }
    schema_description_append_list(sb, schema->multi_argument_functions,
                                   schema->multi_argument_function_count, "/");
    cbm_sb_append(sb, ". Semantics: ");
    schema_description_append_list(sb, schema->semantics, schema->semantic_count, "; ");
    cbm_sb_append(sb, ". Unsupported: ");
    schema_description_append_list(sb, schema->unsupported, schema->unsupported_count, "; ");
    cbm_sb_append(sb, ".");
}
...
static char *build_query_graph_tool_description(cbm_mcp_server_t *srv, const tool_def_t *tool_def) {
    cbm_sb_t sb;
    cbm_sb_init(&sb);
    cbm_sb_append(&sb, tool_def->description);
    schema_description_append_cypher_capabilities(&sb);
    int node_base_count = 0;
    int edge_base_count = 0;
    const char *const *node_base = cbm_store_schema_node_base_properties(&node_base_count);
    const char *const *edge_base = cbm_store_schema_edge_base_properties(&edge_base_count);
    ...
    for (int i = 0; i < schema.rel_pattern_count; i++) {
        ...
        cbm_sb_append(&sb, " RETURN source.qualified_name,target.qualified_name LIMIT 20 [");
        schema_description_append_int(&sb, schema.rel_patterns[i].observed_count);
        cbm_sb_append(&sb, "]");
        free(match);
    }
    cbm_sb_append(
        &sb, ". These are examples, not restrictions: write a custom effective, computationally "
             "efficient query for the current problem.");
    cbm_store_schema_free(&schema);
    return cbm_sb_finish(&sb);
}

static const char *query_graph_tool_description(cbm_mcp_server_t *srv, const tool_def_t *tool_def) {
    if (!srv) {
...
/* Real servers cache by project/publication identity. Stateless catalog calls
 * have no owner to cache against, so the list request owns one generated
 * capability-only description and releases it after JSON serialization. */
static const char *query_graph_catalog_description(cbm_mcp_server_t *srv,
                                                   const tool_def_t *tool_def,
                                                   char **stateless_owned) {
    if (srv) {
        return query_graph_tool_description(srv, tool_def);
    }
    if (!*stateless_owned) {
        *stateless_owned = build_query_graph_tool_description(NULL, tool_def);
    }
    return *stateless_owned ? *stateless_owned : tool_def->description;
}
...
static char *cbm_mcp_tools_list_range(cbm_mcp_server_t *srv, int offset, int limit,
    ...
    bool reveal_hidden = (!classic && srv &&
    ...
    char *stateless_query_graph_description = NULL;
    ...
    char *out = yy_doc_to_str(doc);
    yyjson_mut_doc_free(doc);
    free(stateless_query_graph_description);
    return out;
}
...
cbm_pipeline_t *cbm_mcp_server_active_pipeline(cbm_mcp_server_t *srv) {
    ...
    cbm_pipeline_t *active = srv->active_pipeline;
...
bool cbm_mcp_server_start_overlay_compaction(cbm_mcp_server_t *srv, const char *project,
                                             int max_generations) {
    if (!srv || !project || !project[0] || max_generations < CBM_STORE_COMPACT_ALL_GENERATIONS) {
        return false;
    }
    ...
    if (cbm_thread_create(&srv->overlay_compaction_tid, 0, overlay_compaction_thread, srv) != 0) {
        cbm_mutex_lock(&srv->overlay_compaction_lock);
        srv->overlay_compaction_started = false;
        srv->overlay_compaction_finished = false;
        srv->overlay_compaction_project[0] = '\0';
...
static bool quarantine_corrupt_store(cbm_mcp_server_t *srv, const char *project, const char *path,
    ...
    /* Never touch a database that is not one of ours. A user's own SQLite file
     * sitting at the cache path is not a corrupt cache generation, and moving it
     * would be data loss; api-consolidation added this guard and
     * TEST(resolve_store_leaves_foreign_sqlite_db_untouched) asserts it. Upstream's
...
static const char *parent_project_for_db(const char *project, char *buf, size_t bufsz) {
    enum { DEP_SUFFIX_LEN = 4 };
    const char *dep = strstr(project, ".dep");
    if (dep && (dep[DEP_SUFFIX_LEN] == '.' || dep[DEP_SUFFIX_LEN] == '\0')) {
...
static int collect_db_project_names(const char *dir_path, char *out, size_t out_sz, int *out_listed,
                                    bool *truncated) {
    int count = 0;
    int listed = 0;
    size_t offset = 0;
...
static void add_git_context_json(yyjson_mut_doc *doc, yyjson_mut_val *obj, const char *root_path) {
    ...
    if (ctx.is_git) {
        ...
        if (snapshot_available) {
            ...
            yyjson_mut_obj_add_str(doc, git, "worktree_state", worktree_dirty ? "dirty" : "clean");
            if (worktree_dirty) {
...
static void mcp_index_recovery_action(cbm_mcp_server_t *srv, char *out, size_t out_size) {
    ...
    bool visible = allowed && (!srv || cbm_mcp_tool_mode_is_classic(srv) ||
...
static char *build_project_list_error_srv(cbm_mcp_server_t *srv, const char *reason) {
    ...
    int listed_count = 0;
    int total_count = collect_db_project_names(dir_path, projects, sizeof(projects), &listed_count,
                                               &projects_truncated);
...
static bool add_project_status_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    if (!store) {
        ...
        if (atomic_load_explicit(&srv->autoindex_failed, memory_order_acquire)) {
            yyjson_mut_obj_add_str(doc, root, "detail",
                                   "Automatic indexing failed; graph results are unavailable until "
                                   "indexing succeeds.");
    ...
    bool generation_matches = have_project && have_coverage && project_info.indexed_at &&
    ...
    const char *recording_status = have_coverage && coverage_meta.recording_status
                                       ? coverage_meta.recording_status
                                       : "unknown";
    const char *coverage_status = !have_coverage                              ? "unavailable"
    ...
    bool coverage_visible = mcp_tool_allowed(srv->tool_profile, "check_index_coverage") &&
    ...
    if (!context_root && srv->session_root[0] &&
        (!project || (srv->session_project[0] && strcmp(project, srv->session_project) == 0))) {
    ...
    if (context_root && context_root[0]) {
        ...
        if (ecosystem != CBM_PKG_COUNT) {
            yyjson_mut_obj_add_str(doc, root, "detected_ecosystem", cbm_pkg_manager_str(ecosystem));
        }
    }
 ...
 * References (schemes omitted: scripts/security-audit.sh scans for hardcoded
 * URLs and its comment filter does not reach block-comment continuation lines):
 *   modelcontextprotocol.io/specification/2025-06-18/server/resources
 *   modelcontextprotocol.io/docs/concepts/resources
 *   workos.com/blog/mcp-features-guide  ("resources = application-controlled")
 *
...
static void inject_context_once(yyjson_mut_doc *doc, yyjson_mut_val *root, cbm_mcp_server_t *srv,
    ...
    bool inject_enabled = cbm_config_get_bool(srv->config, CBM_CONFIG_CONTEXT_INJECTION, true);
    if (!inject_enabled)
        return;
    ...
    cbm_schema_info_t schema = {0};
    if (store) {
        mcp_get_current_schema(store, proj, MCP_CYPHER_MATCH_VOCABULARY, &schema, NULL, NULL, NULL);
    }
...
static void toon_append_context_count_table(cbm_sb_t *sb, yyjson_mut_val *ctx, const char *json_key,
                                            const char *toon_key, const char *name_key) {
...
static void toon_append_context_key_functions(cbm_sb_t *sb, yyjson_mut_val *ctx) {
    ...
    cbm_toon_table_header(sb, "_context_key_functions", (int)yyjson_mut_arr_size(array), columns,
                          MCP_COL_2);
    yyjson_mut_arr_iter iter;
    yyjson_mut_arr_iter_init(array, &iter);
    yyjson_mut_val *item = NULL;
    while ((item = yyjson_mut_arr_iter_next(&iter))) {
        ...
        cbm_toon_row_begin(sb);
        cbm_toon_cell_str(sb,
                          qualified_name && yyjson_mut_is_str(qualified_name)
...
static void toon_append_context_model(cbm_sb_t *sb, yyjson_mut_val *root) {
    ...
    toon_append_mut_string(sb, ctx, "pagerank_computed_at", "_context_pagerank_computed_at");
    ...
    if (overlay && yyjson_mut_is_obj(overlay)) {
        ...
        toon_append_mut_int(sb, overlay, "total_nodes_visible", "_context_total_nodes_visible");
    }
...
static cbm_store_t *resolve_project_store(cbm_mcp_server_t *srv, char *raw_project,
...
static bool project_is_path(const char *s) {
    ...
    return s[0] == '/' || s[0] == '~' || (strchr(s, '/') != NULL && s[0] != '*');
}
...
static project_expand_t expand_project_param(cbm_mcp_server_t *srv, char *raw) {
    ...
    if (strncmp(raw, "self", 4) == 0 && (raw[4] == '\0' || raw[4] == '.')) {
        ...
        if (r.mode == MATCH_PREFIX && strchr(r.value, '*'))
            r.mode = MATCH_GLOB;
        return r;
    }
...
static bool project_has_adr(cbm_store_t *store, const char *project, const char *root_path) {
    ...
    int path_len = snprintf(adr_path, sizeof(adr_path), "%s/.codebase-memory/adr.md", root_path);
...
static cbm_store_t *open_validated_cbm_query_store(cbm_mcp_server_t *srv, const char *path,
    ...
    if (!valid) {
        ...
        cbm_log_warn("db.skip", "file", base, "reason", "not_cbm_database", "hint",
                     "File in cache dir lacks codebase-memory-mcp schema. "
...
static char *schema_join_properties(char *const *properties, int property_count,
                                    const char *const *base, int base_count, bool extras_only) {
    cbm_sb_t joined;
    cbm_sb_init(&joined);
    bool first = true;
    for (int i = 0; i < property_count; i++) {
...
static void response_toon_append_freshness(cbm_sb_t *sb, yyjson_mut_val *root) {
    ...
    static const char *const string_keys[] = {
        CBM_MCP_FRESHNESS_STATE_KEY,
        CBM_MCP_FRESHNESS_STALE_SCOPE_KEY,
        CBM_MCP_FRESHNESS_READ_MODEL_KEY,
    };
    static const char *const integer_keys[] = {
        CBM_MCP_FRESHNESS_DIRTY_PENDING_KEY,
        CBM_MCP_FRESHNESS_DIRTY_OVERLAY_READY_KEY,
        "overlay_ready_generations",
        "active_file_tombstones",
        "canonical_nodes_visible",
        "overlay_owned_nodes_visible",
        "total_nodes_visible",
    };
    char key[CBM_SZ_128];
    ...
    static const char *const array_keys[] = {CBM_MCP_FRESHNESS_STALE_VIEWS_KEY, "active_sections"};
...
static char *schema_to_toon(const cbm_schema_info_t *schema, yyjson_mut_val *root) {
    ...
    cbm_toon_scalar_str(&sb, "property_rule",
                        "effective_properties=base_properties+extra_properties");
    ...
    cbm_toon_table_header(&sb, "relationship_patterns", schema->rel_pattern_count, pattern_columns,
                          MCP_COL_2);
    for (int i = 0; i < schema->rel_pattern_count; i++) {
        char *match = schema_relationship_pattern_text(&schema->rel_patterns[i], true);
        if (!match) {
...
static char *handle_get_graph_schema(cbm_mcp_server_t *srv, const char *args) {
    ...
    if (!adr_exists) {
        yyjson_mut_obj_add_str(
            doc, root, "adr_hint",
            "No architecture decision record (ADR) found. Use manage_adr(mode='update') to persist "
            "architectural "
    ...
    char *payload =
        response_format == CBM_MCP_OUTPUT_TOON ? schema_to_toon(&schema, root) : yy_doc_to_str(doc);
    yyjson_mut_doc_free(doc);
    cbm_store_schema_free(&schema);
    free(project);
...
static cbm_store_t *resolve_project_store(cbm_mcp_server_t *srv, char *raw_project,
    ...
    bool session_store_selected =
        db_project && srv->session_project[0] && strcmp(db_project, srv->session_project) == 0;
    ...
    if (!store && may_use_session_root && srv->session_root[0] && cbm_is_dir(srv->session_root)) {
        if (srv->autoindex_active) {
...
static int bm25_bind_overlay_query(sqlite3_stmt *stmt, const char *fts_query, const char *project,
                                   int limit, int offset, const char *file_like) {
...
static char *bm25_search_overlay_active(cbm_store_t *store, const char *project, const char *query,
    ...
    while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        yyjson_mut_obj_add_strcpy(doc, item, "name",
                                  (const char *)sqlite3_column_text(stmt, BM25_OVERLAY_COL_NAME));
        ...
        yyjson_mut_obj_add_strcpy(doc, item, "label",
                                  (const char *)sqlite3_column_text(stmt, BM25_OVERLAY_COL_LABEL));
        yyjson_mut_obj_add_strcpy(doc, item, "file_path",
                                  (const char *)sqlite3_column_text(stmt, BM25_OVERLAY_COL_FILE));
...
static void emit_search_results(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    for (int i = 0; i < out->count; i++) {
        ...
        yyjson_doc *pdoc = pdocs ? enrich_node_properties(doc, item, sr->node.properties_json,
                                                          compact, fields, field_count)
                                 : NULL;
        if (pdoc && pdocs) {
    ...
    if (more && limit > 0) {
        char hint[128];
        snprintf(hint, sizeof(hint), "Use offset:%d and limit:%d for next page (%d total)",
...
typedef enum {
    SEMANTIC_QUERY_OK = 0,
    SEMANTIC_QUERY_INVALID_INPUT,
    SEMANTIC_QUERY_RESOURCE_ERROR,
    SEMANTIC_QUERY_STORE_ERROR,
} semantic_query_status_t;

#ifdef CBM_ENABLE_TEST_SEAMS
static CBM_TLS bool semantic_keyword_allocation_should_fail;

void cbm_mcp_test_fail_next_semantic_keyword_allocation(void) {
    semantic_keyword_allocation_should_fail = true;
}
#endif

static const char **semantic_keywords_allocate(size_t keyword_count) {
#ifdef CBM_ENABLE_TEST_SEAMS
    if (semantic_keyword_allocation_should_fail) {
        semantic_keyword_allocation_should_fail = false;
        return NULL;
    }
#endif
    return malloc(keyword_count * sizeof(const char *));
}

/* Run semantic_query from raw arguments. Keyword pointers borrow the parsed
 * argument document only for the synchronous store call; the pointer array is
 * query-sized and released before return. Runtime is O(Q) here plus the store's
...
static semantic_query_status_t run_semantic_query_core(const char *args, cbm_store_t *store,
    ...
    *out_vcount = 0;
    ...
    semantic_query_status_t status = SEMANTIC_QUERY_OK;
    if (sq_val && !yyjson_is_arr(sq_val)) {
        ...
        if (keyword_count > (size_t)INT_MAX || keyword_count > SIZE_MAX / sizeof(const char *)) {
            ...
            if (!keywords) {
                ...
                yyjson_val *keyword_value = NULL;
                yyjson_arr_foreach(sq_val, array_index, array_count, keyword_value) {
                    if (!yyjson_is_str(keyword_value)) {
                        status = SEMANTIC_QUERY_INVALID_INPUT;
                        break;
                    }
                    keywords[keyword_index++] = yyjson_get_str(keyword_value);
                }
                if (status == SEMANTIC_QUERY_OK) {
                    cbm_vector_result_t *vresults = NULL;
                    int vcount = 0;
                    int sem_limit = limit > 0 ? limit : CBM_SZ_16;
                    if (cbm_store_vector_search(store, project, keywords, (int)keyword_count,
                                                sem_limit, &vresults, &vcount) != CBM_STORE_OK) {
                        cbm_store_free_vector_results(vresults, vcount);
                        status = SEMANTIC_QUERY_STORE_ERROR;
                    } else {
...
static char *semantic_query_error_response(semantic_query_status_t status, cbm_store_t *store) {
    if (status == SEMANTIC_QUERY_INVALID_INPUT) {
        return semantic_query_type_error_response();
    }
    if (status == SEMANTIC_QUERY_STORE_ERROR) {
        cbm_sb_t sb;
        cbm_sb_init(&sb);
        cbm_sb_append(&sb, "Exact semantic search failed: ");
        cbm_sb_append(&sb, cbm_store_error(store));
        cbm_sb_append(&sb, ". No partial semantic or graph results were returned. Retry after "
                           "repairing or rebuilding the semantic index.");
        char *message = cbm_sb_finish(&sb);
        char *response = cbm_mcp_text_result(
            message ? message : "Exact semantic search failed; no partial results were returned.",
            true);
        free(message);
        return response;
    }
    return cbm_mcp_text_result(
        "Unable to prepare the complete semantic_query without truncation. Retry with a "
        "smaller request or after freeing memory; no partial results were returned.",
        true);
}
...
static semantic_query_status_t run_semantic_query(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    int vcount = 0;
    bool present = false;
    semantic_query_status_t status =
    ...
    if (status == SEMANTIC_QUERY_OK && present && project &&
        cbm_store_derived_view_is_stale(store, project, CBM_STORE_DERIVED_VIEW_SEMANTIC_EDGES)) {
        add_stale_derived_view_warning(
            doc, root, CBM_STORE_DERIVED_VIEW_SEMANTIC_EDGES,
            "semantic_edges derived view is stale; semantic_results may be stale.");
    }
    if (status == SEMANTIC_QUERY_OK && vcount > 0) {
...
static bool sg_field_blocked(const char *field) {
    return strcmp(field, "fp") == 0 || strcmp(field, "sp") == 0 || strcmp(field, "bt") == 0;
}
...
static void sg_toon_extra_cells(cbm_sb_t *sb, const char *properties_json,
    ...
    yyjson_doc *properties_doc = properties_json && properties_json[0]
                                     ...
                                     : NULL;
    yyjson_val *properties = properties_doc ? yyjson_doc_get_root(properties_doc) : NULL;
    for (int i = 0; i < field_count; i++) {
        yyjson_val *value =
...
static void sg_lines_str(char *out, size_t out_size, int start_line, int end_line) {
    if (start_line > 0) {
        snprintf(out, out_size, "%d-%d", start_line, end_line > start_line ? end_line : start_line);
    } else {
        out[0] = '\0';
    }
}
...
static void emit_search_results_toon(cbm_sb_t *sb, const cbm_search_output_t *out, int offset,
    ...
    const char *columns[6 + SG_MAX_EXTRA_FIELDS] = {"qn", "label", "file", "lines", "in", "out"};
    int column_count = 6;
    for (int i = 0; i < field_count; i++) {
...
static void emit_semantic_results_toon(cbm_sb_t *sb, const cbm_vector_result_t *results,
                                       int result_count) {
...
static char *append_semantic_query_to_json(const char *base_json, const char *args,
                                           ...
                                           semantic_query_status_t *status) {
    bool semantic_present = cbm_mcp_has_arg(args, "semantic_query");
    if (status) {
        *status = SEMANTIC_QUERY_OK;
    }
    /* The base BM25 document already is the desired output when no semantic
     * query exists. Avoid an otherwise redundant O(B) JSON parse/copy/serialize
     * cycle and O(B) peak memory for a base response of B bytes. */
    if (!base_json || !semantic_present) {
        return NULL;
    }
    ...
    semantic_query_status_t semantic_status =
        run_semantic_query(mdoc, root, args, store, project, limit, NULL, NULL);
    if (semantic_status != SEMANTIC_QUERY_OK) {
        if (status) {
    ...
    yyjson_mut_doc_free(mdoc);
    if (!out && status && semantic_present) {
        *status = SEMANTIC_QUERY_RESOURCE_ERROR;
    }
    return out;
}
...
char *cbm_mcp_add_dirty_file_freshness_to_json(const char *base_json, cbm_store_t *store,
    ...
    yyjson_mut_doc_set_root(mdoc, root);
    cbm_mcp_add_dirty_file_freshness_counts(mdoc, root, pending, overlay_ready, warning_message);
    char *out = yy_doc_to_str(mdoc);
    yyjson_mut_doc_free(mdoc);
    return out;
}
...
static char *handle_search_graph(cbm_mcp_server_t *srv, const char *args) {
    ...
    if (query && query[0]) {
        int q_limit =
            cbm_mcp_get_positive_int_arg(args, "limit", cfg_search_limit, CBM_DEFAULT_SEARCH_LIMIT);
        ...
        bool q_overlay_ready = project && project[0] &&
                               cbm_store_get_overlay_node_view_summary(
                                   store, project, &q_overlay_summary) == CBM_STORE_OK &&
        ...
        bool q_require_json = legacy_json || q_overlay_ready || q_has_dirty_state ||
                              cbm_mcp_has_arg(args, "semantic_query");
        char *bm25_json = q_overlay_ready
                              ? bm25_search_overlay_active(store, project, query, q_file_pattern,
                                                           q_limit, q_offset, &q_overlay_summary)
                              : bm25_search(store, project, query, q_file_pattern, q_limit,
                                            q_offset, !q_require_json);
        free(q_file_pattern);
        ...
        if (bm25_json) {
            semantic_query_status_t semantic_status = SEMANTIC_QUERY_OK;
            char *composed_json = append_semantic_query_to_json(bm25_json, args, store, project,
                                                                q_limit, &semantic_status);
            free(query);
            if (semantic_status != SEMANTIC_QUERY_OK) {
                free(pe.value);
                free(bm25_json);
                return semantic_query_error_response(semantic_status, store);
            }
    ...
    if (relationship && !validate_edge_type(relationship)) {
        ...
        return cbm_mcp_text_result(
            ...
            "e.g. CALLS, DEFINES, IMPORTS (max 64 chars).\"}",
            true);
    }
     ...
     */
    if (!sort_by && srv->config) {
        const char *cfg_sort = cbm_config_get(srv->config, "default_sort_by", NULL);
        if (cfg_sort && cfg_sort[0])
            sort_by = heap_strdup(cfg_sort);
    }
    ...
    if (sort_by && strcmp(sort_by, "relevance") != 0 && strcmp(sort_by, "name") != 0 &&
        ...
        snprintf(errbuf, sizeof(errbuf),
                 ...
                 "\"hint\":\"Valid values: relevance, name, degree, calls, linkrank\"}",
                 sort_by);
        free(label);
        free(name_pattern);
        free(qn_pattern);
    ...
    if (search_mode && strcmp(search_mode, "full") != 0 && strcmp(search_mode, "summary") != 0) {
        ...
        if (strcmp(search_mode, "compact") == 0 || strcmp(search_mode, "files") == 0) {
            ...
            snprintf(errbuf, sizeof(errbuf),
                     "{\"error\":\"mode '%s' is only valid for the search_code tool (source grep), "
                     "not search_graph\","
                     "\"hint\":\"For graph mode use mode='full' or mode='summary'. "
                     "To reduce token output in graph mode, pass compact=true (boolean).\"}",
                     search_mode);
        } else {
            snprintf(errbuf, sizeof(errbuf),
                     "{\"error\":\"invalid mode '%s'\","
                     "\"hint\":\"Valid values for graph mode: full, summary. "
                     "For source grep, use the search_code tool (modes: compact, full, files).\"}",
                     search_mode);
        }
        free(label);
        free(name_pattern);
    ...
    bool include_dependencies =
        cbm_mcp_get_bool_arg_default(args, "include_dependencies", cfg_inc_deps);
    ...
    params.degree_mode = srv->config ? cbm_config_get(srv->config, "degree_mode", NULL) : NULL;
    /* Dep-ranking: default false (deps rank last). Config key
     * search_disable_dep_ranking=true → pure relevance (deps may rank high). */
    params.disable_dep_ranking =
        srv->config ? cbm_config_get_bool(srv->config, CBM_CONFIG_SEARCH_DISABLE_DEP_RANKING, false)
                    : false;
    params.limit = limit;
    ...
    if (exclude_count < 0) {
        ...
        return cbm_mcp_text_result(
            ...
            "\"hint\":\"Retry with fewer exclude patterns or a smaller request.\"}",
            true);
    }
    ...
    if (!legacy_json) {
        ...
        if (semantic_status == SEMANTIC_QUERY_OK) {
            ...
            if (!semantic_only) {
                ...
                int search_rc = is_summary && toon_overlay_ready
            ...
            char *result = cbm_mcp_text_result(text ? text : "out of memory",
                                               text == NULL || toon_search_failed);
            free(text);
            return result;
        }
        /* Semantic failures are atomic: do not fall through to graph results. */
        cbm_store_free_vector_results(vresults, vcount);
        if (fields_owner) {
            yyjson_doc_free(fields_owner);
        }
    ...
    bool overlay_active_edges_requested =
        relationship || include_connected || exclude_entry_points || min_degree >= 0 ||
        max_degree >= 0 ||
    ...
    if (is_summary) {
        ...
        yyjson_mut_obj_add_str(doc, root, "hint",
                               "mode='summary' returns counts only. Use mode='full' with "
                               "compact=true for node records.");
    }
    ...
    if (!graph_search_failed && out.total == 0) {
        ...
        if (pe.mode == MATCH_PREFIX && pe.value) {
            ...
            is_dep_search =
                (strstr(pe.value, ".dep.") != NULL || strstr(pe.value, ".dep%") != NULL);
        }
        if (is_dep_search) {
    ...
    /* Semantic (vector) search: append semantic_results only after the full,
     * validated keyword array has completed successfully. */
    bool sq_present = false;
...
static void hint_append_name_list(cbm_sb_t *msg, const char *title, int total,
    ...
    bool truncated = total > CBM_STORE_SCHEMA_HINT_VOCAB_LIMIT;
    int n = truncated ? CBM_STORE_SCHEMA_HINT_VOCAB_LIMIT : total;
    cbm_sb_append(msg, title);
    for (int i = 0; i < n; i++) {
        cbm_sb_append(msg, i ? ", " : "");
        cbm_sb_append(msg, name_at(schema, i));
    }
    cbm_sb_append(msg, truncated ? " (+more)." : ".");
}
...
static char *query_graph_no_rows_hint(cbm_store_t *store, const char *view_project,
    ...
    if (unknown_count > 0) {
        ...
        cbm_sb_free(&unknown);
        cbm_sb_append(&msg, "Query returned no results, but the referenced labels and edge types "
...
static char *handle_query_graph(cbm_mcp_server_t *srv, const char *args) {
    ...
    if (!query)
        query = cbm_mcp_get_string_arg(args, "query"); /* backward compat */
    ...
    char *project = pe.value;
    int cfg_max_rows =
        cbm_config_get_int(srv->config, CBM_CONFIG_QUERY_MAX_ROWS, CBM_DEFAULT_QUERY_MAX_ROWS);
    int max_rows =
        cbm_mcp_has_arg(args, "max_rows") ? cbm_mcp_get_int_arg(args, "max_rows", 0) : cfg_max_rows;
    ...
    if (!query) {
        free(project);
        return cbm_mcp_text_result("{\"error\":\"query is required\","
                                   "\"hint\":\"Pass a Cypher query string, e.g. MATCH (n:Function) "
    ...
    /* This value intentionally crosses result cleanup: the byte-cap response
     * reports rows materialized even though no result rows are returned. */
...
static char *handle_delete_project(cbm_mcp_server_t *srv, const char *args) {
    ...
    if (!name) {
        return cbm_mcp_text_result("{\"error\":\"project_name is required\","
                                   "\"hint\":\"Pass the project name to delete. Use list_projects "
    ...
    if (!path[0]) {
        ...
        free(name);
        return cbm_mcp_text_result(
            "{\"status\":\"delete_failed\",\"error\":\"project path too long\"}", true);
    }
        ...
        if (db) {
            ...
            if (excl_count < 0) {
                ...
                if (!kf_sql_heap) {
                    ...
                    if (sqlite3_prepare_v2(db, kf_sql, -1, &kf_stmt, NULL) == SQLITE_OK) {
                        if (project)
                            sqlite3_bind_text(kf_stmt, 1, project, -1, SQLITE_TRANSIENT);
                        if (path_scoped) {
                        ...
                        while (sqlite3_step(kf_stmt) == SQLITE_ROW) {
                            ...
                            if (qn)
                                yyjson_mut_obj_add_strcpy(doc, kf, "qualified_name", qn);
                            if (lbl && lbl[0])
                                yyjson_mut_obj_add_strcpy(doc, kf, "label", lbl);
                            if (fp && fp[0])
                                yyjson_mut_obj_add_strcpy(doc, kf, "file_path", fp);
 ...
 * array is the graph induced by all visited nodes, not a BFS tree; selecting
 * the first incident edge can therefore attach a lateral edge's args/evidence
 * to an unrelated hop. Build one shortest-predecessor index over the complete
 * traversal before pagination, then share it across TOON/JSON and args/evidence.
 *
 * Expected runtime is O(V + E) and memory is O(V), versus the former
 * O(page_rows * E) repeated scans. Load stays <= 1/2, so linear probing has
 * expected O(1) lookup without a fixed graph-size ceiling. */
typedef struct {
    bool occupied;
    int64_t node_id;
    int hop;
...
typedef struct {
    trace_edge_slot_t *slots;
    size_t capacity;
    bool outbound;
} trace_edge_index_t;

enum {
    TRACE_EDGE_INDEX_MIN_CAPACITY = 8,
    TRACE_EDGE_INDEX_LOAD_DENOMINATOR = 2,
    TRACE_EDGE_HASH_SHIFT_FIRST = 30,
    TRACE_EDGE_HASH_SHIFT_SECOND = 27,
    TRACE_EDGE_HASH_SHIFT_FINAL = 31,
};

/* SplitMix64 finalizer constants give stable, portable distribution for
 * sequential SQLite node IDs; these are algorithm constants, not tuneables. */
static const uint64_t TRACE_EDGE_HASH_MULTIPLIER_FIRST = UINT64_C(0xbf58476d1ce4e5b9);
static const uint64_t TRACE_EDGE_HASH_MULTIPLIER_SECOND = UINT64_C(0x94d049bb133111eb);

static size_t trace_edge_hash(int64_t node_id) {
    uint64_t value = (uint64_t)node_id;
    value ^= value >> TRACE_EDGE_HASH_SHIFT_FIRST;
    value *= TRACE_EDGE_HASH_MULTIPLIER_FIRST;
    value ^= value >> TRACE_EDGE_HASH_SHIFT_SECOND;
    value *= TRACE_EDGE_HASH_MULTIPLIER_SECOND;
    value ^= value >> TRACE_EDGE_HASH_SHIFT_FINAL;
    return (size_t)value;
}

static trace_edge_slot_t *trace_edge_index_slot(trace_edge_index_t *index, int64_t node_id,
                                                bool create) {
    if (!index || !index->slots || index->capacity == 0) {
        return NULL;
    }
    size_t mask = index->capacity - SKIP_ONE;
    size_t slot = trace_edge_hash(node_id) & mask;
    for (size_t probe = 0; probe < index->capacity; probe++) {
        trace_edge_slot_t *entry = &index->slots[(slot + probe) & mask];
        if (!entry->occupied) {
            if (!create) {
                return NULL;
            }
...
static const trace_edge_slot_t *trace_edge_index_find(const trace_edge_index_t *index,
                                                      int64_t node_id) {
    if (!index || !index->slots || index->capacity == 0) {
        return NULL;
    }
    ...
    size_t slot = trace_edge_hash(node_id) & mask;
    for (size_t probe = 0; probe < index->capacity; probe++) {
        const trace_edge_slot_t *entry = &index->slots[(slot + probe) & mask];
        if (!entry->occupied) {
            return NULL;
        }
        if (entry->node_id == node_id) {
...
static bool trace_predecessor_better(const trace_edge_index_t *index,
                                     const cbm_edge_info_t *candidate,
                                     const cbm_edge_info_t *current) {
    if (!current) {
        return true;
    }
    int64_t candidate_parent = index->outbound ? candidate->source_id : candidate->target_id;
    int64_t current_parent = index->outbound ? current->source_id : current->target_id;
    if (candidate_parent != current_parent) {
        return candidate_parent < current_parent;
    }
    int type_order =
        strcmp(candidate->type ? candidate->type : "", current->type ? current->type : "");
    if (type_order != 0) {
        return type_order < 0;
    }
    return strcmp(candidate->properties_json ? candidate->properties_json : "",
                  current->properties_json ? current->properties_json : "") < 0;
}

static void trace_edge_index_free(trace_edge_index_t *index);

static bool trace_edge_index_build(trace_edge_index_t *index, const cbm_traverse_result_t *tr,
                                   const cbm_node_t *roots, int root_count, bool outbound) {
    memset(index, 0, sizeof(*index));
    index->outbound = outbound;
    size_t canonical_root_count = (size_t)(root_count > 0 ? root_count : 0);
    size_t visited_count = (size_t)tr->visited_count;
    if (canonical_root_count > SIZE_MAX / TRACE_EDGE_INDEX_LOAD_DENOMINATOR) {
        return false;
    }
    /* Reserve one query-local overlay root for each canonical candidate so
     * inferred roots do not violate the hash table's target load factor. */
    size_t root_slots = canonical_root_count * TRACE_EDGE_INDEX_LOAD_DENOMINATOR;
    if (visited_count > SIZE_MAX - root_slots) {
        return false;
    }
    size_t slot_count = visited_count + root_slots;
    if (slot_count > SIZE_MAX / TRACE_EDGE_INDEX_LOAD_DENOMINATOR) {
        return false;
    }
    size_t required = slot_count * TRACE_EDGE_INDEX_LOAD_DENOMINATOR;
    size_t capacity = TRACE_EDGE_INDEX_MIN_CAPACITY;
    while (capacity < required) {
        if (capacity > SIZE_MAX / TRACE_EDGE_INDEX_LOAD_DENOMINATOR) {
            return false;
        }
        capacity *= TRACE_EDGE_INDEX_LOAD_DENOMINATOR;
    }
    ...
    for (int i = 0; i < root_count; i++) {
        trace_edge_slot_t *slot = trace_edge_index_slot(index, roots[i].id, true);
        if (!slot) {
            trace_edge_index_free(index);
            return false;
        }
        slot->hop = 0;
    }
    for (int i = 0; i < tr->visited_count; i++) {
        trace_edge_slot_t *slot = trace_edge_index_slot(index, tr->visited[i].node.id, true);
        if (!slot) {
            trace_edge_index_free(index);
            return false;
        }
        if (tr->visited[i].hop < slot->hop) {
            slot->hop = tr->visited[i].hop;
        }
    }
    for (int i = 0; i < tr->edge_count; i++) {
        const cbm_edge_info_t *edge = &tr->edges[i];
        int64_t parent_id = outbound ? edge->source_id : edge->target_id;
        int64_t child_id = outbound ? edge->target_id : edge->source_id;
        trace_edge_slot_t *parent = trace_edge_index_slot(index, parent_id, false);
        trace_edge_slot_t *child = trace_edge_index_slot(index, child_id, false);
        /* Overlay traversals can use query-local logical root IDs that differ
         * from the canonical root candidates. An edge into a hop-1 node proves
         * that its otherwise-unvisited parent is a traversal root. */
        if (!parent && child && child->hop == SKIP_ONE) {
            parent = trace_edge_index_slot(index, parent_id, true);
            if (!parent) {
                trace_edge_index_free(index);
                return false;
            }
        ...
        if (!parent || !child || parent->hop == INT_MAX || child->hop != parent->hop + SKIP_ONE) {
            continue;
        }
        if (trace_predecessor_better(index, edge, child->predecessor)) {
            child->predecessor = edge;
        }
    }
    return true;
}

static const cbm_edge_info_t *trace_edge_index_get(const trace_edge_index_t *index,
                                                   int64_t node_id) {
    const trace_edge_slot_t *slot = trace_edge_index_find(index, node_id);
    return slot ? slot->predecessor : NULL;
}
...
/* Return the borrowed serialized args array on one selected predecessor edge. */
static const char *bfs_edge_args(const cbm_edge_info_t *edge, size_t *out_len) {
...
static uint64_t trace_params_hash(const char *project, const char *symbol, const char *direction,
    ...
    const char *parts[] = {project ? project : "", symbol ? symbol : "", direction ? direction : "",
                           mode ? mode : ""};
...
static void trace_cursor_encode(const trace_cursor_t *cursor, char *out, size_t out_size) {
    snprintf(out, out_size, "c1.%c.%s.%016llx.%d.%lld", cursor->leg, cursor->generation,
             (unsigned long long)cursor->query_hash, cursor->hop, (long long)cursor->node_id);
}

static const char *trace_cursor_decode(const char *token, const char *generation,
                                       uint64_t expected_hash, trace_cursor_t *out) {
    memset(out, 0, sizeof(*out));
    if (!token || strncmp(token, "c1.", 3) != 0 || (token[3] != 'o' && token[3] != 'i') ||
        token[4] != '.') {
    ...
    if (!generation_end || (size_t)(generation_end - generation_start) >= sizeof(out->generation)) {
        return "invalid_cursor: unrecognized token; rerun without cursor";
    }
    ...
    if (sscanf(generation_end + 1, "%16llx.%d.%lld", &parsed_hash, &out->hop, &parsed_node_id) !=
        3) {
        return "invalid_cursor: unrecognized token; rerun without cursor";
    }
...
static bool trace_row_visible(const cbm_node_hop_t *row, bool include_tests, char **exclude_likes) {
...
static bool trace_has_visible_after(const cbm_traverse_result_t *tr, int start, bool include_tests,
                                    char **exclude_likes) {
...
/* Parse evidence from the same deterministic shortest-predecessor edge used
 * for data-flow args. Index construction is O(V + E) runtime/O(V) memory and
 * each row lookup plus bounded property parse is expected O(1). */
static bool bfs_edge_evidence(const cbm_edge_info_t *edge, const char **class_out,
                              double *confidence_out) {
    const char *pj = edge ? edge->properties_json : NULL;
    const char *key = pj ? strstr(pj, "\"strategy\"") : NULL;
    if (!key) {
        return false;
    }
    const char *open = strchr(key + strlen("\"strategy\""), '"');
    if (!open) {
        return false;
    }
    open++;
    ...
    *class_out = strategy_class;
    /* Absent confidence is -1 so genuine 0.0 remains distinguishable. */
    *confidence_out = -1.0;
    const char *confidence = strstr(pj, "\"confidence\"");
    if (confidence) {
        const char *colon = strchr(confidence, ':');
        if (colon) {
            *confidence_out = strtod(colon + SKIP_ONE, NULL);
        }
    }
    return true;
}
...
static void bfs_to_toon_table(cbm_sb_t *sb, const char *key, cbm_traverse_result_t *tr,
                              ...
                              bool include_evidence, const trace_edge_index_t *edge_index) {
    int visible = 0;
    for (int i = 0; i < tr->visited_count; i++) {
    ...
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        if (data_flow) {
            size_t alen = 0;
            const cbm_edge_info_t *edge = trace_edge_index_get(edge_index, tr->visited[i].node.id);
            const char *ea = bfs_edge_args(edge, &alen);
        ...
        if (include_evidence) {
            ...
            const cbm_edge_info_t *edge = trace_edge_index_get(edge_index, tr->visited[i].node.id);
            if (bfs_edge_evidence(edge, &strategy_class, &confidence)) {
                cbm_toon_cell_str(sb, strategy_class, false);
                if (confidence >= 0.0) {
                    char confidence_text[CBM_SZ_32];
                    snprintf(confidence_text, sizeof(confidence_text), "%.2f", confidence);
                    cbm_toon_cell_str(sb, confidence_text, false);
                } else {
                    cbm_toon_cell_str(sb, "", false);
                }
            } else {
...
static void trace_append_nodes(cbm_mcp_server_t *srv, yyjson_mut_doc *doc, yyjson_mut_val *arr,
                               const cbm_traverse_result_t *tr, bool compact, bool include_tests,
                               bool risk_labels, bool data_flow, bool include_evidence,
                               char **exclude_likes, const trace_edge_index_t *edge_index) {
    ...
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        if (data_flow) {
            size_t args_len = 0;
            const cbm_edge_info_t *edge = trace_edge_index_get(edge_index, hop->node.id);
            const char *edge_args = bfs_edge_args(edge, &args_len);
        ...
        if (include_evidence) {
            ...
            const cbm_edge_info_t *edge = trace_edge_index_get(edge_index, hop->node.id);
            if (bfs_edge_evidence(edge, &strategy_class, &confidence)) {
                yyjson_mut_obj_add_str(doc, item, "strategy", strategy_class);
                if (confidence >= 0.0) {
...
static void bfs_union_same_name(cbm_store_t *store, const cbm_node_t *nodes, int node_count,
                                ...
                                int depth, int max_results, cbm_traverse_result_t *out) {
    memset(out, 0, sizeof(*out));
    int visited_capacity = 0;
    int edge_capacity = 0;
    for (int node_index = 0; node_index < node_count; node_index++) {
        cbm_traverse_result_t traversal = {0};
        cbm_store_bfs(store, nodes[node_index].id, direction, edge_types, edge_type_count, depth,
                      max_results, &traversal);
        ...
        for (int i = 0; i < traversal.visited_count; i++) {
            ...
            if (out->visited_count >= visited_capacity) {
                visited_capacity = visited_capacity ? visited_capacity * 2 : CBM_SZ_8;
                out->visited =
                    safe_realloc(out->visited, (size_t)visited_capacity * sizeof(*out->visited));
            }
        ...
        for (int i = 0; i < traversal.edge_count; i++) {
            ...
            for (int j = 0; j < out->edge_count; j++) {
                duplicate = out->edges[j].source_id == traversal.edges[i].source_id &&
            ...
            if (out->edge_count >= edge_capacity) {
                ...
                out->edges = safe_realloc(out->edges, (size_t)edge_capacity * sizeof(*out->edges));
            }
    ...
    if (out->visited_count > 1) {
        qsort(out->visited, (size_t)out->visited_count, sizeof(*out->visited), node_hop_cmp_hop_id);
    }
}
...
static int clamp_mcp_depth(int depth, const char *tool_name) {
    ...
    if (depth > depth_limit) {
        ...
        cbm_log_warn("mcp.depth_capped", "tool", tool_name, "requested", requested, "cap", limit);
        return depth_limit;
    }
    return depth;
}
...
static char *handle_trace_path(cbm_mcp_server_t *srv, const char *args) {
    ...
    /* Evidence is opt-in because it adds two response columns. Args and
     * evidence share one O(V + E)-runtime/O(V)-memory predecessor index. */
    ...
    if (!func_name && !qn_input) {
        ...
        free(qn_input);
        return cbm_mcp_text_result("{\"error\":\"function_name or qualified_name is required\","
                                   "\"hint\":\"Pass the name of a function to trace, e.g. "
                                   "{\\\"function_name\\\":\\\"main\\\"}\"}",
                                   true);
    }

    /* Validate direction enum */
    if (direction && strcmp(direction, "inbound") != 0 && strcmp(direction, "outbound") != 0 &&
        strcmp(direction, "both") != 0) {
        char errbuf[256];
        snprintf(errbuf, sizeof(errbuf),
                 "{\"error\":\"invalid direction '%s'\","
                 "\"hint\":\"Valid values: inbound, outbound, both\"}",
                 direction);
        free(func_name);
        free(qn_input);
        free(raw_project);
    ...
    if (trace_mode && strcmp(trace_mode, "calls") != 0 && strcmp(trace_mode, "data_flow") != 0 &&
        strcmp(trace_mode, "cross_service") != 0) {
        char errbuf[256];
        snprintf(errbuf, sizeof(errbuf),
                 "{\"error\":\"invalid mode '%s'\","
                 "\"hint\":\"Valid values: calls, data_flow, cross_service\"}",
                 trace_mode);
        free(func_name);
        free(qn_input);
        free(raw_project);
    ...
    if (qn_input) {
        ...
        int qn_rc = overlay_ready_for_trace
                        ? cbm_store_find_node_by_qn_overlay_view(store, project, qn_input, &qn_tmp)
    ...
    if (qn_node) {
        ...
        if (overlay_ready_for_trace) {
            cbm_store_find_nodes_by_name_overlay_view(
                store, project, func_name ? func_name : (qn_input ? qn_input : ""), &nodes,
                &node_count);
        } else {
            cbm_store_find_nodes_by_name(store, project,
                                         func_name ? func_name : (qn_input ? qn_input : ""), &nodes,
                                         &node_count);
        }
    }
    ...
    if (node_count == 0 && func_name) {
        ...
        if (search_rc == 0 && sout.count > 0) {
            ...
            if (overlay_ready_for_trace) {
                cbm_store_find_nodes_by_name_overlay_view(
                    store, found_project ? found_project : project, sout.results[0].node.name,
                    &nodes, &node_count);
            } else {
                cbm_store_find_nodes_by_name(store, found_project ? found_project : project,
    ...
    char **edge_types_user =
        cbm_mcp_get_string_array_arg(args, "edge_types", &edge_type_count_user);
    if (edge_type_count_user < 0) {
        ...
        return cbm_mcp_text_result(
            ...
            "\"hint\":\"Retry with fewer edge_types or a smaller request.\"}",
            true);
    }
    ...
    if (have_cursor && legacy_generation) {
        cursor_error =
            "cursor_unsupported: reindex this project to enable generation-safe pagination";
    } else if (have_cursor) {
    ...
    bool do_outbound =
        strcmp(effective_direction, "outbound") == 0 || strcmp(effective_direction, "both") == 0;
    bool do_inbound =
        strcmp(effective_direction, "inbound") == 0 || strcmp(effective_direction, "both") == 0;
    ...
    if (do_outbound) {
        if (overlay_trace_requested) {
            int bfs_rc = cbm_store_bfs_overlay_view(store, project, nodes[sel].qualified_name,
                                                    "outbound", edge_types, edge_type_count, depth,
                                                    MCP_BFS_LIMIT_MAX, &tr_out);
    ...
    if (do_inbound) {
        if (overlay_trace_requested) {
            int bfs_rc = cbm_store_bfs_overlay_view(store, project, nodes[sel].qualified_name,
                                                    "inbound", edge_types, edge_type_count, depth,
    ...
    bool out_more =
        do_outbound && trace_has_visible_after(&tr_out, out_end, include_tests, exclude_likes);
    bool in_more =
        do_inbound && trace_has_visible_after(&tr_in, in_end, include_tests, exclude_likes);
    ...
    bool has_dirty_counts =
        get_dirty_file_counts(store, project, &dirty_pending, &dirty_overlay_ready);
    bool derived_stale = (do_outbound && (tr_out.pagerank_stale || tr_out.linkrank_stale)) ||
    ...
    trace_edge_index_t out_edge_index = {0};
    trace_edge_index_t in_edge_index = {0};
    bool need_edge_index = data_flow || include_evidence;
    bool edge_index_ok =
        (!need_edge_index || !do_outbound ||
         trace_edge_index_build(&out_edge_index, &tr_out, nodes, node_count, true)) &&
        (!need_edge_index || !do_inbound ||
         trace_edge_index_build(&in_edge_index, &tr_in, nodes, node_count, false));
    char *json = NULL;
    bool response_is_error = false;
    if (!edge_index_ok) {
        json = heap_strdup("out of memory preparing shortest-path trace args and evidence");
        response_is_error = true;
    } else if (!trace_legacy_json) {
        cbm_sb_t sb;
        cbm_sb_init(&sb);
        ...
        if (more_rows) {
            ...
            if (next_cursor[0]) {
                ...
                cbm_toon_scalar_str(
                    &sb, "hint", "More rows exist; reindex to enable safe cursors or raise limit.");
            }
        }
        json = cbm_sb_finish(&sb);
    } else {
        ...
        if (node_count > 1) {
            ...
            int candidate_limit =
                node_count < MCP_TRACE_CANDIDATE_LIMIT ? node_count : MCP_TRACE_CANDIDATE_LIMIT;
        ...
        if (do_outbound) {
            ...
            trace_append_nodes(srv, doc, callees, &view_out, compact, include_tests, risk_labels,
                               data_flow, include_evidence, exclude_likes, &out_edge_index);
        ...
        if (do_inbound) {
            ...
            trace_append_nodes(srv, doc, callers, &view_in, compact, include_tests, risk_labels,
                               data_flow, include_evidence, exclude_likes, &in_edge_index);
        ...
        add_derived_freshness_warnings(
            ...
            (do_outbound && tr_out.linkrank_stale) || (do_inbound && tr_in.linkrank_stale), false);
    ...
    trace_edge_index_free(&out_edge_index);
    trace_edge_index_free(&in_edge_index);
    if (do_outbound) {
        cbm_store_traverse_free(&tr_out);
    }
    if (do_inbound) {
...
static char *get_project_root(cbm_mcp_server_t *srv, const char *project) {
    ...
    if (!project || project[0] == '\0') {
        ...
        /* Path-based arg: convert to slug (shared helper, same logic as expand_project_param Rule
         * 0) */
    ...
    char *root = project_root_from_store(store, slug);
    free(slug_owned);
    return root;
}
 ...
 *
 * The name is normalized through cbm_project_name_from_path, NOT used verbatim:
 * handle_index_repository normalizes it the same way when it writes the project,
 * so using the raw string here would fail to find a project whose name needed
 * normalizing. */
static char *handle_cross_repo_mode(cbm_mcp_server_t *srv, const char *repo_path,
    ...
    /* A missing SOURCE is reported; a missing TARGET is skipped and counted.
     * pass_cross_repo.h:35-37 states the contract exactly: "a missing source
     * sets source_missing and runs nothing; a missing named target is skipped
     * and counted in targets_missing. Neither creates a database."
     *
     * These are opposite outcomes and must stay distinct. Skipping is right for
     * targets — one unindexed project in a wildcard list must not fail the whole
     * run — but wrong for the source, where nothing was matched FROM and there
     * is no partial result to report. Without this branch, source_missing was
     * SET by pass_cross_repo.c:1203 and read by nobody, so a cross-repo run
     * against an unindexed source returned a success envelope with every edge
     * count at zero: indistinguishable from a repository that genuinely shares
     * no interfaces. result.failed does not cover it, since nothing failed —
     * the run correctly declined to start. */
    ...
    /* Every NAMED target was missing, so nothing could be matched against.
     *
     * This is the third case in the contract, distinct from the two above.
     * Skipping a missing target is right when other targets remain — the count
     * in targets_missing tells the caller what was left out. But when the skip
     * consumes the entire target list, "success, 0 projects scanned, 0 edges"
     * is indistinguishable from a real scan that found nothing in common, and
     * the caller has no way to learn that its target names were all wrong.
     *
     * Guarded on targets_missing > 0 so the wildcard form is unaffected: "*"
     * contributes no missing NAMES, so an empty cache still reports an honest
     * zero-result scan rather than an error. */
    if (result.targets_missing > 0 && result.projects_scanned == 0) {
        char msg[CBM_SZ_512];
        snprintf(msg, sizeof(msg),
                 "{\"error\":\"none of the requested target projects are indexed (%d missing); "
                 "cross-repo-intelligence matches existing indexes only\",\"hint\":\"Run "
                 "list_projects to see indexed projects, index the targets you meant, then rerun "
    ...
    if (result.failed) {
        /* Keep the actionable message: it names the project, says what
         * cross-repo-intelligence matches against, and gives the exact next
         * command. The terse form told the caller nothing they could act on. */
        char msg[CBM_SZ_512];
        snprintf(msg, sizeof(msg),
                 "{\"error\":\"project '%s' is not indexed, invalid, or missing; "
                 "cross-repo-intelligence matches existing indexes only\",\"hint\":\"Run "
                 "index_repository on this repo_path (with the same name, if any) first, then "
                 "rerun cross-repo-intelligence. Run list_projects to see indexed projects.\"}",
                 project);
        free(project);
        return cbm_mcp_text_result(msg, true);
    }
 ...
 * to invalidate. */
/* One function, two names before the merge: api-consolidation exported it as
 * the publication notification (declared in mcp.h), upstream kept it private as
 * invalidate_cached_store. The bodies were identical, so the public name wins
 * and the private twin is gone. The name describes the EVENT; the body is the
 * deferred invalidation that event triggers. */
...
/* Request-thread transition from a cached query handle to an owned writer.
 * Closing is O(SQLite connection state) runtime and releases the reader before
 * publication; no second cache-invalidating implementation is needed. */
static void close_cached_store_for_write(cbm_mcp_server_t *srv) {
    if (!srv) {
        return;
    }
    atomic_store(&srv->store_stale, true);
...
static char *index_run_supervised(cbm_mcp_server_t *srv, const char *args) {
    ...
    if (wr.cancellation_requested ||
        (srv && atomic_load_explicit(&srv->pipeline_cancel_requested, memory_order_acquire) != 0)) {
    ...
    /* A recovery run that got far enough to publish owns the authoritative response
     * (retry at the wr2 arm, quarantine at the wrp arm). Return it: reporting a
     * contained crash after the crasher was successfully quarantined tells the caller
     * the index failed when it actually succeeded, and drops the skipped[] evidence
     * naming the culprit file. Assigning resp here without returning it also leaked it. */
    if (resp) {
        return resp;
    }
    if (unsafe_terminal) {
...
static char *handle_index_repository(cbm_mcp_server_t *srv, const char *args) {
    ...
    /* The supervisor gate deliberately lives further down, after the raw repo_path
     * has been resolved against the session root and canonicalized, so the worker
     * receives an absolute path it cannot reinterpret against its own working
     * directory, and so the workspace-authorization check runs before any work is
     * handed out. Gating here instead would forward the caller's raw JSON: a
     * relative "repo" would then resolve against the worker's cwd and publish a
     * different project than the one the parent validated. */
    char *repo_path = cbm_mcp_get_string_arg(args, "repo_path");
    ...
    if (!repo_path) {
        ...
        return cbm_mcp_text_result(
            ...
            "\"hint\":\"Pass the absolute path to the project root directory.\"}",
            true);
    }
    ...
    /* Reject an unrecognized mode instead of silently indexing in full mode and
     * reporting success: a typo, or an internal-only spelling such as "dep", used
     * to select CBM_MODE_FULL here, so the caller got a different index than the
     * one requested with no way to tell. The accepted list is derived from
     * CBM_INDEX_MODE_TABLE (src/discover/discover.h), never restated. */
    cbm_index_mode_t mode = CBM_MODE_FULL;
    if (mode_str && mode_str[0]) {
        bool caller_selectable = false;
        if (!cbm_index_mode_from_name(mode_str, &mode, &caller_selectable) || !caller_selectable) {
            char accepted[CBM_SZ_64];
            cbm_index_mode_accepted(accepted, (int)sizeof(accepted));
            char message[CBM_SZ_256];
            snprintf(message, sizeof(message),
                     "mode must be %s, got '%s'; omit mode to index in full mode", accepted,
                     mode_str);
            /* The project mutation is already held at this point, so release it
             * exactly as the cancellation path below does; returning without it
             * would leave the project locked against every later mutation. */
            mcp_project_mutation_end(srv, mutation_project);
            free(mutation_project);
            free(repo_path);
            free(mode_str);
    ...
    if (!p) {
        ...
        free(repo_path);
        CBM_PROF_END("index_repository", "TOTAL", prof_index_total);
        return cbm_mcp_text_result("{\"error\":\"failed to create indexing pipeline\","
                                   "\"hint\":\"Check that repo_path exists and is readable. The "
    ...
    yyjson_mut_obj_add_str(doc, root, "publish_kind", cbm_pipeline_publish_kind_name(publish_kind));
    ...
    if (publication_committed) {
        ...
        cbm_store_t *owned_writable_store = NULL;
        cbm_store_t *store = cbm_mcp_writable_existing_store(resolved_store, &owned_writable_store);
        ...
        if (store) {
            ...
            int deps_reindexed = 0;
            int dep_owner_rc = cbm_mcp_finish_index_publication(
                srv, project_name, repo_path, store, args, graph_changed, publish_kind,
                incremental_fallback, &dep_stats, &deps_reindexed);
            ...
            if (eco != CBM_PKG_COUNT)
                yyjson_mut_obj_add_str(doc, root, "detected_ecosystem", cbm_pkg_manager_str(eco));
            ...
            if (!adr_exists) {
                yyjson_mut_obj_add_str(
                    ...
                    "Project indexed. Consider creating an Architecture Decision Record: "
                    "explore the codebase with get_architecture(aspects=['all']), then use "
                    "manage_adr(mode='update') to persist architectural insights across MCP server "
                    "runs.");
            }
    ...
    if (rc == 0 && publish_kind == CBM_PIPELINE_PUBLISH_INCREMENTAL_OVERLAY &&
        ...
        bool started = cbm_mcp_server_start_overlay_compaction(srv, project_name, compact_max);
        yyjson_mut_obj_add_str(doc, root, "overlay_compaction_policy",
                               CBM_CONFIG_OVERLAY_COMPACTION_POLICY_AFTER_PUBLISH);
        yyjson_mut_obj_add_int(doc, root, "overlay_compaction_max_generations", compact_max);
        ...
        const char *compact_status =
            started ? "started"
                    : (cbm_mcp_server_overlay_compaction_active(srv) ? "already_running"
                                                                     : "not_started");
        yyjson_mut_obj_add_str(doc, root, "overlay_compaction_status", compact_status);
    }
...
static char *build_snippet_response(cbm_mcp_server_t *srv, cbm_node_t *node,
                                    ...
                                    cbm_node_t *alternatives, int alt_count, int max_lines,
                                    const char *mode, bool compact,
    ...
    if (root_path && node->file_path) {
        ...
        int path_len = abs_path ? snprintf(abs_path, apsz, "%s/%s", root_path, node->file_path)
                                : CBM_NOT_FOUND;
        bool path_ok =
        ...
        if (path_ok && !stale_span) {
            if (signature_mode) {
                ...
                int head_count =
                    (max_lines * CBM_SNIPPET_HEAD_PERCENT) / CBM_SNIPPET_PERCENT_DENOMINATOR;
    ...
    yyjson_mut_obj_add_str(doc, root_obj, "source_origin", snippet_dep ? "dependency" : "project");
    if (snippet_dep) {
...
static char *handle_get_code_snippet(cbm_mcp_server_t *srv, const char *args) {
    ...
    if (!qn) {
        ...
        return cbm_mcp_text_result(
            ...
            "\"hint\":\"Pass a symbol qualified name, e.g. "
            "{\\\"qualified_name\\\":\\\"myapp.src.main.handle_request\\\"}. "
            "Use search_graph to find qualified names.\"}",
            true);
    }
    ...
    if (snippet_mode && strcmp(snippet_mode, "full") != 0 &&
        ...
        snprintf(errbuf, sizeof(errbuf),
                 "{\"error\":\"invalid mode '%s'\","
                 "\"hint\":\"Valid values: full, signature, head_tail\"}",
                 snippet_mode);
        free(qn);
        free(project);
        free(snippet_mode);
    ...
    REQUIRE_STORE_EX(store, project,
                     (free(qn), free(snippet_mode), qn = NULL, snippet_mode = NULL));
    ...
    bool overlay_ready_for_snippet = ready_overlay_exists &&
                                     cbm_store_get_overlay_node_view_summary(
                                         store, eff_project, &overlay_summary) == CBM_STORE_OK &&
                                     cbm_store_overlay_node_view_has_ready_rows(&overlay_summary);
    ...
    if (rc == CBM_STORE_OK) {
        char *result = build_snippet_response(srv, &node, NULL /*exact*/, include_neighbors, NULL,
                                              0, max_lines, snippet_mode, compact, active_summary);
    ...
    /* Nothing found */
    {
        ...
        snprintf(
            ...
            "\"hint\":\"Use search_graph with name_pattern to find the correct qualified_name.\"}",
            qn);
        free(qn);
        free(project);
        free(snippet_mode);
...
static bool search_code_git_worktree_available(const char *root_path) {
    char cmd[CBM_SZ_2K];
#ifdef _WIN32
    int n = snprintf(cmd, sizeof(cmd), "git -C \"%s\" rev-parse --is-inside-work-tree 2>NUL",
                     root_path);
#else
    int n = snprintf(cmd, sizeof(cmd), "git -C \"%s\" rev-parse --is-inside-work-tree 2>/dev/null",
                     root_path);
#endif
...
static bool build_grep_cmd(char *cmd, size_t cmd_sz, bool use_regex, bool case_sensitive,
    ...
    if (scan_mode == SEARCH_CODE_SCAN_GIT_GREP) {
        n = snprintf(cmd, cmd_sz,
                     "git -C \"%s\" grep -n%s --untracked %s -f \"%s\" -- . 2>/dev/null", root_path,
                     ci_flag, flag, tmpfile);
    } else if (scan_mode == SEARCH_CODE_SCAN_FILELIST_GREP) {
        (void)file_pattern;
        /* #687: filelist is NUL-delimited so spaces remain within one path. */
        n = snprintf(cmd, cmd_sz, "xargs -0 grep -Hn%s %s -f '%s' -- < '%s' 2>/dev/null", ci_flag,
                     flag, tmpfile, filelist);
    } else {
        if (file_pattern) {
            n = snprintf(cmd, cmd_sz, "grep -Hrn%s %s --include='%s' -f '%s' '%s' 2>/dev/null",
...
static char *handle_search_code(cbm_mcp_server_t *srv, const char *args) {
    ...
    bool has_exact_filter_path =
        !file_pattern &&
        extract_exact_path_filter(path_filter, exact_filter_path, sizeof(exact_filter_path));
    char *mode_str = cbm_mcp_get_string_arg(args, "mode");
        ...
        if (strcmp(mode_str, "full") == 0) {
            ...
            if (strcmp(mode_str, "summary") == 0) {
                snprintf(mode_warning_msg, sizeof(mode_warning_msg),
                         "mode='summary' is only valid for graph mode (default dispatch), not "
                         "source grep. "
                         "For source grep use mode='compact' (default), 'full', or 'files'. "
                         "Using mode='compact' for this request.");
            } else {
                snprintf(mode_warning_msg, sizeof(mode_warning_msg),
                         "unknown mode '%s' for source grep; valid values: compact (default), "
                         "full, files. "
                         "Using mode='compact' for this request.",
                         mode_str);
            }
    ...
    if (!pattern) {
        ...
        return cbm_mcp_text_result(
            ...
            "\"hint\":\"Pass a text pattern or regex (with regex:true) to search source code.\"}",
            true);
    }
    ...
    if (!search_scratch_open(&scratch, pattern)) {
        ...
        free(project);
        free(file_pattern);
        return cbm_mcp_text_result("{\"error\":\"search failed: could not create temp file\","
    ...
    if (!build_grep_cmd(cmd, sizeof(cmd), use_regex, case_sensitive, scan_mode, file_pattern,
        ...
        free(project);
        free(file_pattern);
        return cbm_mcp_text_result("{\"error\":\"search failed: grep command too long\","
                                   "\"hint\":\"Use a shorter project path or file_pattern.\"}",
                                   true);
    }
    ...
    bool needs_freshness_json =
        overlay_ready_for_code || dirty_pending > 0 || dirty_overlay_ready > 0;
    char *result = NULL;
...
static int mcp_run_git_argv(cbm_mcp_server_t *srv, const char *repo_path,
    ...
    if (srv->command_test_hook && !srv->command_test_hook(srv->command_test_context, git_args[0])) {
        return -1;
    }
...
enum {
    DETECT_HUNK_CAP = 4096,
    DETECT_HUNK_MAX_BYTES = DETECT_HUNK_CAP * CBM_SZ_1K,
};

/* Parse contained Git diff outputs into one bounded hunk set. Runtime is
 * O(diff_bytes + hunk_count), with O(diff_bytes + hunk_count) transient memory
 * up to the named safety ceilings. Oversized, unreadable, or truncated input
 * disables only precision scoping; callers retain whole-file seed recall. */
static cbm_changed_hunk_t *detect_parse_hunk_outputs(const cbm_git_output_t *outputs,
                                                     size_t output_count, int *hunk_count) {
    *hunk_count = 0;
    size_t total_bytes = 0;
    for (size_t i = 0; i < output_count; i++) {
        if (outputs[i].size > (size_t)DETECT_HUNK_MAX_BYTES - total_bytes) {
            cbm_log_info("detect_changes.hunks", "action", "scoping_disabled", "reason",
                         "diff_bytes_exceeded");
            return NULL;
        }
        total_bytes += outputs[i].size;
    }
    if (total_bytes == 0) {
        return NULL;
    }
    char *buffer = malloc(total_bytes + SKIP_ONE);
    if (!buffer) {
        return NULL;
    }
    size_t offset = 0;
    bool read_ok = true;
    for (size_t i = 0; i < output_count && read_ok; i++) {
        FILE *fp = outputs[i].path[0] ? cbm_fopen(outputs[i].path, "rb") : NULL;
        if (!fp) {
            read_ok = false;
            break;
        }
        size_t read_count = fread(buffer + offset, SKIP_ONE, outputs[i].size, fp);
        bool file_read_ok = read_count == outputs[i].size && ferror(fp) == 0;
        bool close_ok = fclose(fp) == 0;
        read_ok = file_read_ok && close_ok;
        offset += read_count;
    }
    if (!read_ok) {
        free(buffer);
    ...
    buffer[offset] = '\0';
    cbm_changed_hunk_t *hunks = malloc((size_t)DETECT_HUNK_CAP * sizeof(*hunks));
    if (!hunks) {
        free(buffer);
        return NULL;
    }
    *hunk_count = cbm_parse_hunks(buffer, hunks, DETECT_HUNK_CAP);
    free(buffer);
    if (*hunk_count >= DETECT_HUNK_CAP) {
        cbm_log_info("detect_changes.hunks", "action", "scoping_disabled", "reason",
                     "hunk_cap_reached");
        free(hunks);
        *hunk_count = 0;
...
static char *handle_detect_changes(cbm_mcp_server_t *srv, const char *args) {
    ...
    const char *const status_args[] = {"status", "--porcelain", "--untracked-files=normal", NULL};
    const char *const committed_hunk_args[] = {"diff", "--unified=0", revision, "--", NULL};
    const char *const unstaged_hunk_args[] = {"diff", "--unified=0", "--", NULL};
    const char *const *commands[] = {committed_args, unstaged_args, status_args,
                                     committed_hunk_args, unstaged_hunk_args};
    enum {
        DETECT_CHANGED_COMMAND_COUNT = 3,
        DETECT_HUNK_COMMAND_COUNT = 2,
        DETECT_GIT_COMMAND_COUNT = DETECT_CHANGED_COMMAND_COUNT + DETECT_HUNK_COMMAND_COUNT,
    };
    ...
    int command_count = want_symbols ? DETECT_GIT_COMMAND_COUNT : DETECT_CHANGED_COMMAND_COUNT;
    for (int i = 0; i < command_count; i++) {
        if (mcp_run_git_argv(srv, root_path, commands[i], &git_outputs[i], &git_results[i]) != 0) {
            if (i < DETECT_CHANGED_COMMAND_COUNT) {
                git_run = -1;
                break;
            }
            /* Hunk scoping is a precision optimization. A contained diff
             * failure retains the complete whole-file seeding path. */
            cbm_git_output_cleanup(&git_outputs[i]);
            continue;
        }
    ...
    cbm_changed_hunk_t *hunks =
        want_symbols ? detect_parse_hunk_outputs(&git_outputs[DETECT_CHANGED_COMMAND_COUNT],
                                                 DETECT_HUNK_COMMAND_COUNT, &hunk_count)
                     : NULL;

    bool outputs_read = true;
    for (int i = 0; i < DETECT_CHANGED_COMMAND_COUNT; i++) {
        if (!detect_collect_changed_output(&git_outputs[i], store, project, want_symbols, &files,
                                           &file_count, &file_cap, &seeds, &seed_count, &seed_cap,
                                           hunks, hunk_count)) {
            outputs_read = false;
            break;
        }
    ...
    free(hunks);
    int git_status = git_results[DETECT_CHANGED_COMMAND_COUNT - 1].exit_code;
    mcp_git_outputs_cleanup(git_outputs, DETECT_GIT_COMMAND_COUNT);
    if (!outputs_read) {
        for (int i = 0; i < file_count; i++) {
            free(files[i]);
    ...
    char merge_base[64] = "";
    {
        ...
        int mb_run = mcp_run_git_argv(srv, root_path, merge_base_args, &mb_output, &mb_result);
...
static cbm_store_t *open_adr_store_for_write(cbm_mcp_server_t *srv, cbm_store_t *resolved,
    ...
    close_cached_store_for_write(srv);
...
static char *handle_index_dependencies(cbm_mcp_server_t *srv, const char *args) {
    ...
    if (!packages_val || !yyjson_is_arr(packages_val) || yyjson_arr_size(packages_val) == 0) {
        ...
        free(raw_project);
        free(pkg_mgr_str);
        return cbm_mcp_text_result("{\"error\":\"packages[] is required\"}", true);
    }
    ...
    if (!store) {
        ...
        free(pkg_mgr_str);
        return cbm_mcp_text_result(
            resolved_store ? "{\"error\":\"project store could not be opened read-write\"}"
...
/* Renamed from the branch's public cbm_mcp_handle_tool into upstream's private
 * dispatcher slot: the public wrapper below adds the request scope around it. */
static char *dispatch_tool(cbm_mcp_server_t *srv, const char *tool_name, const char *args_json) {
    ...
    if (strcmp(tool_name, "_hidden_tools") == 0) {
        bool changed =
            srv && !srv->hidden_tools_revealed && !srv->client_requires_static_tool_catalog;
...
static void detect_session(cbm_mcp_server_t *srv) {
    ...
    if (srv->session_project[0] == '\0' || strcmp(srv->session_project, ".") == 0 ||
        strcmp(srv->session_project, "..") == 0) {
        cbm_log_warn("session.invalid_name", "derived", srv->session_project, "cwd",
                     srv->session_root, "hint", "Cannot derive valid project name from CWD");
...
static void *autoindex_thread(void *arg) {
    ...
    if (rc == 0) {
        ...
        /* cbm_store_open_path_existing is an external ownership boundary;
         * cppcheck does not model its nullable heap-return contract. */
        // cppcheck-suppress knownConditionTrueFalse
        if (store) {
            (void)cbm_mcp_finish_index_publication(srv, srv->session_project, srv->session_root,
                                                   store, NULL, graph_changed, publish_kind,
                                                   incremental_fallback, NULL, NULL);
            cbm_store_close(store);
        } else {
            /* No writable handle for the freshly published database: dependency
             * indexing and rank refresh are skipped, but sessions must still be
             * told their cached store is stale. */
            cbm_mcp_server_notify_index_published(srv);
        }
...
static bool db_is_stale(const char *db_path, const char *repo_path, int max_age_seconds) {
    ...
    if (max_age_seconds > 0) {
        time_t now = time(NULL);
        if (now > db_mtime && (now - db_mtime) > max_age_seconds)
            return true;
    }

    /* Check git HEAD commit time vs DB mtime */
    if (!validate_search_path_arg(repo_path))
        return false;
    char cmd[1024];
#ifdef _WIN32
...
static bool cbm_mcp_reindex_on_startup(cbm_mcp_server_t *srv) {
    return cbm_config_get_effective_bool(srv ? srv->config : NULL, CBM_CONFIG_REINDEX_ON_STARTUP,
                                         false);
}

static int cbm_mcp_reindex_stale_seconds(cbm_mcp_server_t *srv) {
    return cbm_config_get_effective_int(srv ? srv->config : NULL, CBM_CONFIG_REINDEX_STALE_SECONDS,
                                        0);
}
...
bool cbm_mcp_auto_index_within_configured_limit(const char *root_path, int configured_limit,
                                                int *file_count_out) {
    /* See mcp_internal.h: 0 (and any non-positive value) is the configuration
     * spelling for "no limit", so no count is needed and nothing is refused. */
    ...
    return cbm_mcp_auto_index_within_file_limit(root_path, configured_limit, file_count_out);
}
...
static void maybe_auto_index(cbm_mcp_server_t *srv) {
    ...
    if (!auto_index) {
        ...
        cbm_log_info(
            "autoindex.skip", "reason", "disabled", "hint",
    ...
    /* No diagnostic needed here: cbm_mcp_auto_index_within_limit already emits
     * autoindex.skip with reason, files, limit AND path, and distinguishes a
     * traversal failure (file_count_failed) from an over-limit repo. That is
     * strictly more than upstream's inline log carried. */
...
static const char *active_project_name(cbm_mcp_server_t *srv) {
    if (srv->current_project)
        return srv->current_project;
...
static void build_resource_schema(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    bool active_schema_failed = false;
    cbm_schema_info_t schema = {0};
    mcp_get_current_schema(store, proj, MCP_CYPHER_MATCH_VOCABULARY, &schema, &overlay_summary,
                           &used_active_schema, &active_schema_failed);
    ...
    if (exclude_csv && exclude_csv[0]) {
        ...
        while (tok && pos < (int)sizeof(sql) - 128) {
            ...
            if (like) {
                ...
                free(like);
                if (safe) {
                    pos += snprintf(sql + pos, sizeof(sql) - pos, "AND n.file_path NOT LIKE '%s' ",
                                    safe);
                    free(safe);
                }
            }
...
static void build_resource_architecture(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    bool arch_loaded =
        proj && proj[0] &&
        cbm_store_get_architecture(store, proj, resource_aspects,
                                   (int)(sizeof(resource_aspects) / sizeof(resource_aspects[0])),
                                   &arch, 0, 1.0) == CBM_STORE_OK;
    if (arch_loaded) {
        add_architecture_languages_json(doc, root, &arch);
        add_architecture_entry_points_json(doc, root, &arch);
        add_architecture_routes_json(doc, root, &arch);
        ...
        if (!sql) {
            ...
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                ...
                if (qn)
                    yyjson_mut_obj_add_strcpy(doc, kf, "qualified_name", qn);
                if (label && label[0])
                    yyjson_mut_obj_add_strcpy(doc, kf, "label", label);
                if (fp && fp[0])
                    yyjson_mut_obj_add_strcpy(doc, kf, "file_path", fp);
                add_pagerank_val(doc, kf, rank);
                yyjson_mut_arr_add_val(kf_arr, kf);
...
static void build_resource_status(yyjson_mut_doc *doc, yyjson_mut_val *root,
    ...
    if (db && proj) {
        ...
        if (sqlite3_prepare_v2(db, "SELECT name FROM projects WHERE name LIKE ?1 ORDER BY name", -1,
                               &stmt, NULL) == SQLITE_OK) {
...
char *cbm_mcp_server_handle(cbm_mcp_server_t *srv, const char *line) {
    ...
    if (!req.has_id) {
        if (req.method && strcmp(req.method, "notifications/cancelled") == 0) {
            /* Cancel BOTH: the request scope (every cancellable operation in
             * this dispatch) and any in-flight index_repository pipeline. */
            cbm_mutex_lock(&srv->active_request_lock);
            cbm_pipeline_t *active = srv->active_pipeline;
...
int cbm_mcp_server_run(cbm_mcp_server_t *srv, FILE *in, FILE *out) {
    ...
    /* No line buffer here: cbm_mcp_read_message() owns its own bounded buffer,
     * which is what enforces MCP_MAX_HEADER_SIZE / MCP_MAX_MESSAGE_SIZE. */
    ...
    for (;;) {
        ...
        free(message);
        if (resp) {
            write_protocol_json(out, resp, content_length_framed);
            free(resp);

...
/* Source-built distribution: self-update is disabled. The binary must never
 * reach out to the network; rebuild and reinstall from the repository. */
int cbm_cmd_update(int argc, char **argv) {
    (void)argc;
    (void)argv;
    (void)fprintf(stderr, "error: self-update is disabled in this source-built distribution.\n"
                          "Rebuild and reinstall from the repository instead:\n"

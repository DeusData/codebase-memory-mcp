...
static const char skill_content[] =
    ...
    "2. `query_graph` output is capped by query_max_output_bytes. Use LIMIT when it helps query "
    "efficiency during exploration; omit it when the full result is necessary, and set "
...
static const char codex_instructions_content[] =
    ...
    "boundary, "
    "retry that operation with escalation instead of treating the block as a project failure. "
    "MCP approval and shell sandbox authorization are separate.\n";
...
static const char agent_instructions_content[] =
    ...
    "4. `query_graph` — write problem-specific Cypher for effective, computationally efficient "
    "structural answers; examples and LIMIT are optional guidance\n"

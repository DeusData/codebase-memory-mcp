...
int cbm_replace_binary(const char *path, const unsigned char *data, int len, int mode) {
    ...
    if (stat(path, &st_check) == 0) {
        ...
        if (cbm_unlink(path) != 0) {
            ...
            if (cbm_rename_replace(path, old_path) != 0) {
                return CLI_ERR;
            }
#else
            return CLI_ERR;
...
static const char skill_content[] =
    ...
    "2. `query_graph` output is bounded by query_max_output_bytes. Use LIMIT when it helps "
    "exploration efficiency; omit it when full results are necessary, and set max_output_bytes=0 "
    "only when uncapped output is appropriate.\n"

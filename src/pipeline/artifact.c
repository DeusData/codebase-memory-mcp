...
static bool git_head_hash(const char *repo_path, char *buf, size_t bufsz) {
    ...
    if (command_length < 0 || (size_t)command_length >= sizeof(cmd)) {
...
int cbm_artifact_import(const char *repo_path, const char *cache_db_path) {
    ...
    /* Drop destination WAL/SHM only after the imported DB passes integrity and
     * immediately before publishing it: stale sidecars could otherwise replay
     * the previous cache onto the imported database (#897). Preserve the native
     * Windows replacement path from #400/#486 and the original MoveFileEx
     * replacement contribution from #492. */

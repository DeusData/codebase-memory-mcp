...
int cbm_pipeline_pass_calls(cbm_pipeline_ctx_t *ctx, const cbm_file_info_t *files, int file_count) {
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (files[i].language == CBM_LANG_GDSCRIPT) {
            gdscript_source = read_file(path, &gdscript_source_len);
        }
        ...
        for (int c = 0; c < result->calls.count; c++) {
            ...
            if (files[i].language == CBM_LANG_GDSCRIPT) {
                ...
                if (has_anchor_qn && allow_same_script_signal &&
                    ...
                    res = cbm_registry_resolve(ctx->registry, gdscript_target_qn, module_qn,
                                               imp_keys, imp_vals, imp_count);
                }
                if ((!res.qualified_name || res.qualified_name[0] == '\0') &&
                    gdscript_receiver_signal_target_qn(ctx, result, call, module_qn, imp_keys,
                                                       imp_vals, imp_count,
                                                       (has_anchor_qn &&
                                                        gdscript_anchor_matches_module(anchor_qn,
                                                                                       module_qn))
                                                           ? gdscript_source
                                                           : NULL,
            ...
            if (!res.qualified_name || res.qualified_name[0] == '\0') {
                res = cbm_registry_resolve(ctx->registry, call->callee_name, module_qn, imp_keys,
                                           imp_vals, imp_count);
            }

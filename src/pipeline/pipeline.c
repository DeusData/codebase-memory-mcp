...
static int run_extraction_phase(cbm_pipeline_t *p, cbm_pipeline_ctx_t *ctx,
    ...
    const char *requested_mode = cbm_safe_getenv("CBM_FORCE_PIPELINE_MODE", requested_mode_buf,
                                                 sizeof(requested_mode_buf), NULL);
    cbm_pipeline_mode_selection_t mode_selection = {0};
    int rc = cbm_pipeline_select_mode(requested_mode, worker_count, file_count, &mode_selection);

...
static void sem_process_def_edges(cbm_pipeline_ctx_t *ctx, const CBMDefinition *def,
                                  const char *module_qn, const char **imp_keys,
                                  const char **imp_vals, int imp_count,
                                  const cbm_file_info_t *file_info,
                                  int *inherits_count, int *decorates_count) {
        ...
        for (int b = 0; def->base_classes[b]; b++) {
            ...
            if (file_info && file_info->language == CBM_LANG_GDSCRIPT) {
                cbm_resolution_t base_res =
                    cbm_registry_resolve(ctx->registry, base_name, module_qn, imp_keys, imp_vals,
                                        imp_count);
                ...
                base_qn = resolve_as_class(ctx->registry, base_name, module_qn, imp_keys, imp_vals,
                                          imp_count);
            }
    ...
    } else if (file_info && file_info->language == CBM_LANG_GDSCRIPT && node->label &&
               strcmp(node->label, "Class") == 0 && node->qualified_name &&
               gdscript_anchor_matches_module(node->qualified_name, module_qn)) {
        int fallback_len = 0;
        char *fallback_source = file_info->path ? read_file(file_info->path, &fallback_len) : NULL;
        if (fallback_source) {
            char module_path[CBM_SZ_512];
            if (gdscript_preload_extends_base_from_source(fallback_source, module_path,
                ...
                const cbm_gbuf_node_t *base_node = base_qn ? cbm_gbuf_find_by_qn(ctx->gbuf, base_qn)
                                                           : NULL;
                if (base_node && node->id != base_node->id) {
                    cbm_gbuf_insert_edge(ctx->gbuf, node->id, base_node->id, "INHERITS", "{}");
                    (*inherits_count)++;
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        for (int d = 0; d < result->defs.count; d++) {
            sem_process_def_edges(ctx, &result->defs.items[d], module_qn, imp_keys, imp_vals,
                                  imp_count, &files[i], &inherits_count, &decorates_count);
        }

...
static char *read_file(const char *path, int *out_len, long *out_size,
    ...
    /* Tree-sitter's lexer may inspect a small lookahead window beyond the
     * logical UTF-8 end. Match the other extraction paths: zero-padding keeps
     * that implementation read in-bounds under ASan. */
...
static bool pp_err_add(pp_err_list_t *list, const char *path, const char *reason,
    ...
    char *owned_path = pp_err_dup(path);
    char *owned_reason = pp_err_dup(reason);
    char *owned_phase = pp_err_dup(phase);
    if ((path && !owned_path) || (reason && !owned_reason) || (phase && !owned_phase)) {
        free(owned_path);
        free(owned_reason);
        free(owned_phase);
        return false;
    }
    list->items[list->count].path = owned_path;
    list->items[list->count].reason = owned_reason;
    list->items[list->count].phase = owned_phase;
    list->count++;
    return true;
}
...
static void append_json_string(char *buf, size_t bufsize, size_t *pos, const char *key,
    ...
    size_t required = strlen(key) + escaped_len + PP_JSON_FIELD_OVERHEAD;
...
static void append_json_str_array(char *buf, size_t bufsize, size_t *pos, const char *key,
    ...
    for (int i = 0; arr[i]; i++) {
        required += cbm_json_escaped_len(arr[i]) + PP_ESC_SPACE + (i > 0 ? SKIP_ONE : 0);
    }
    if (*pos + required + PP_ESC_SPACE > bufsize) {
...
typedef struct {
    ...
    _Atomic int worker_failed; /* allocation/merge failure: abort publication */
...
static void extract_worker(int worker_id, void *ctx_ptr) {
    ...
    while (SKIP_ONE) {
        ...
        if (cbm_index_is_quarantined(fi->rel_path)) {
            ...
            if (!pp_err_add(errs, fi->rel_path, reason, phase)) {
                atomic_store_explicit(&ec->worker_failed, 1, memory_order_relaxed);
            }
            continue;
        }
        ...
        if (!source) {
            if (rst == CBM_READ_OVERSIZED) {
                ...
                if (!pp_err_add(errs, fi->rel_path, reason, "oversized")) {
                    atomic_store_explicit(&ec->worker_failed, 1, memory_order_relaxed);
                }
        ...
        CBMFileResult *result =
                ...
                ? cbm_pipeline_extract_objectscript_export(source, source_len, ec->project_name,
                                                           fi->rel_path, ec->extract_timeout_micros,
                                                           ec->extract_macros, ec->macro_table,
        ...
        if (!result || result->has_error) {
            ...
            ws->errors++;
            if (!pp_err_add(errs, fi->rel_path, "extract failed", "extract")) {
                atomic_store_explicit(&ec->worker_failed, 1, memory_order_relaxed);
            }
            continue;
        }
        ...
        if (result->parse_incomplete) {
            ...
            if (!pp_err_add(errs, fi->rel_path,
...
static int merge_pkg_entries(cbm_pipeline_ctx_t *ctx, cbm_pkg_entries_t *pkg_entries,
                             int worker_count) {
    if (!pkg_entries) {
        return CBM_NOT_FOUND;
    }
    ...
    if (!ctx->pkgmap_preseeded) {
        ...
        cbm_pkgmap_scan_repo(ctx->repo_path, &pkg_entries[0], ctx->excluded_dirs,
                             ctx->excluded_count);
        int entry_count = 0;
        for (int i = 0; i < worker_count; i++) {
            entry_count += pkg_entries[i].count;
        }
...
int cbm_parallel_extract_ex(cbm_pipeline_ctx_t *ctx, const cbm_file_info_t *files, int file_count,
    ...
    if (!ctx || file_count < 0 || worker_count <= 0 ||
        (file_count > 0 && (!files || !result_cache || !shared_ids || !ctx->cancelled))) {
        return CBM_NOT_FOUND;
    }
    ...
    extract_ctx_t ec = {
        ...
        .extract_macros = cbm_pipeline_mode_extracts_macro_nodes(ctx->mode),
        .extract_timeout_micros = cbm_pipeline_ctx_extract_timeout(ctx),
        .err_lists = err_lists,
    ...
    atomic_init(&ec.worker_failed, 0);
    ...
    for (int i = 0; i < worker_count; i++) {
        ...
        if (workers[i].local_gbuf) {
            int worker_merge_rc = cbm_gbuf_merge(ctx->gbuf, workers[i].local_gbuf);
            if (worker_merge_rc != 0) {
                merge_rc = worker_merge_rc;
            } else {
    ...
    int pkgmap_rc = merge_pkg_entries(ctx, pkg_entries, worker_count);
...
static int register_and_link_def(cbm_pipeline_ctx_t *ctx, const CBMDefinition *def, const char *rel,
     ...
     * Interface/Enum/Type/Trait), plus variables and fields for READS/WRITES.
     * The shared predicate is the source of truth and must stay aligned with
     * pass_definitions.c and pipeline_incremental.c. */
...
static void emit_service_edge(cbm_gbuf_t *gbuf, const cbm_gbuf_node_t *source,
                              ...
                              bool suppress_plain_calls, const char *rel, CBMLanguage lang) {
    ...
    if (svc == CBM_SVC_ROUTE_REG) {
        ...
        if (route_path && suffix_only_route_reg && handlerless_http_client) {
            /* A generic dotted verb with no handler is an HTTP client call, not
             * a route registration (commit a0a320f5). Emit it directly so valid
             * one-segment paths do not depend on the conservative URL fallback. */
    ...
    /* A resolved bare fetch is a local/imported shadow. The genuine global API
     * is emitted only from the empty-resolution path (#856). */
...
static const CBMResolvedCall *lsp_idx_lookup(const CBMHashTable *index, const CBMCall *call,
                                             bool exact_site, double confidence_floor,
                                             bool *key_built, bool *ambiguous) {
    if (key_built) {
        *key_built = false;
    }
    if (ambiguous) {
...
static void resolve_file_calls(resolve_ctx_t *rc, resolve_worker_state_t *ws, CBMFileResult *result,
    ...
    double confidence_floor = rc->lsp_confidence_floor > 0.0 ? rc->lsp_confidence_floor
        ...
        if (lsp_exact_idx || lsp_legacy_idx) {
            for (int i = 0; i < result->resolved_calls.count; i++) {
                ...
                if (rc_e->kind != CBM_RESOLVED_INVOCATION || !rc_e->caller_qn || !rc_e->callee_qn ||
                    rc_e->confidence < confidence_floor) {
                    continue;
                }
                bool exact_site =
    ...
    for (int c = 0; c < result->calls.count; c++) {
         ...
         * Exact occurrence spans are indexed first; legacy rows and JVM tail
         * reconciliation retain the authoritative linear fallback. */
        ...
        if (cbm_pipeline_source_site_present(call->site_start_byte, call->site_end_byte)) {
            lsp = lsp_idx_lookup(lsp_exact_idx, call, true, confidence_floor, &exact_key_built,
                                 &exact_key_ambiguous);
        }
        ...
        if (!lsp && !exact_index_authoritative) {
            ...
            lsp = cbm_pipeline_find_lsp_resolution_with_floor_in_graph(
                &result->resolved_calls, call, confidence_floor, allow_tail, rc->main_gbuf,
                rc->project_name);
        }
        if (!lsp && !call->requires_lsp_resolution && exact_index_authoritative &&
            !exact_key_ambiguous) {
            lsp = lsp_idx_lookup(lsp_legacy_idx, call, false, confidence_floor, &legacy_key_built,
                                 NULL);
        }
        ...
        _rc_t0 = extract_now_ns();
        cbm_pipeline_try_field_type_hint(rc->registry, rc->main_gbuf, &res, call->callee_name,
                                         source_node->id);
        ...
        /* Type-aware LSP weak-method suppression. The receiver-aware guard must
         * NOT drop this call here: doing so would also skip the #523
         ...
         * leaves lsp_* resolutions untouched. */
...
static char *pp_lsp_site_key(CBMArena *arena, CBMResolvedKind kind, const char *caller,
                             uint32_t start, uint32_t end, CBMSourceOrigin origin) {
    return caller && cbm_pipeline_source_site_present(start, end)
               ? cbm_arena_sprintf(arena, "%u\x1f%s\x1f%u:%u\x1f%u", (unsigned)kind, caller, start,
                                   end, (unsigned)origin)
               : NULL;
}

static int pp_qualified_lsp_site_count(const CBMFileResult *result, double confidence_floor) {
    CBMArena keys;
    cbm_arena_init(&keys);
    uint32_t capacity = (uint32_t)(result->calls.count + result->usages.count +
                                   result->resolved_calls.count + CBM_SZ_16);
    CBMHashTable *sites = cbm_ht_create(capacity);
    CBMHashTable *counted = cbm_ht_create(capacity);
    if (!sites || !counted) {
        cbm_ht_free(sites);
        cbm_ht_free(counted);
    ...
    for (int i = 0; i < result->calls.count; i++) {
        ...
        char *key =
            pp_lsp_site_key(&keys, CBM_RESOLVED_INVOCATION, call->enclosing_func_qn,
                            call->site_start_byte, call->site_end_byte, call->source_origin);
        if (key) {
            cbm_ht_set(sites, key, key);
        }
    }
    for (int i = 0; i < result->usages.count; i++) {
        ...
        char *key =
            pp_lsp_site_key(&keys, CBM_RESOLVED_CALL_REFERENCE, usage->enclosing_func_qn,
                            usage->site_start_byte, usage->site_end_byte, usage->source_origin);
        if (key) {
            cbm_ht_set(sites, key, key);
        }
    }
    ...
    for (int i = 0; i < result->resolved_calls.count; i++) {
        ...
        if (!resolved->callee_qn || resolved->confidence < confidence_floor) {
            continue;
        }
        char *key =
            pp_lsp_site_key(&keys, resolved->kind, resolved->caller_qn, resolved->site_start_byte,
                            resolved->site_end_byte, resolved->source_origin);
        if (key && cbm_ht_has(sites, key) && !cbm_ht_has(counted, key)) {
            cbm_ht_set(counted, key, key);
            count++;
        }
    }
    cbm_ht_free(sites);
...
static bool pp_has_pending_lsp_site(const CBMFileResult *result, double confidence_floor) {
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const CBMResolvedCall *resolved = &result->resolved_calls.items[i];
        ...
        if (resolved->reason || resolved->confidence < confidence_floor) {
            return true;
        }
    }
    return false;
...
static void resolve_worker(int worker_id, void *ctx_ptr) {
    ...
    if (!ws->local_edge_buf) {
        ...
        if (!ws->local_edge_buf) {
            atomic_store_explicit(&rc->worker_failed, 1, memory_order_relaxed);
            cbm_destroy_thread_parser();
            cbm_slab_destroy_thread();
            return;
        }
    ...
    while (SKIP_ONE) {
        ...
        int qualified_lsp_sites = pp_qualified_lsp_site_count(result, confidence_floor);
        bool pending_lsp_site = pp_has_pending_lsp_site(result, confidence_floor);
        ...
        int import_map_rc = cbm_pxc_build_import_map(rc->main_gbuf, rc->project_name, rel, lang,
        ...
        if (cross_lsp_eligible) {
            ...
            if (lsp_source && lsp_source_len > 0) {
                ...
                if (rc->module_def_index && imp_count > 0) {
                    ...
                    import_refine_defs = cbm_pxc_filter_defs_for_file(
                        rc->module_def_index, rc->all_defs, lang, result->namespace_name,
                        def_module, imp_vals, imp_count, &filtered_count, &filter_succeeded);
                    if (filter_succeeded) {
                        import_refine_def_count = filtered_count;
                    }
                }
                ...
                cbm_pxc_dispatch_file(lang, result, lsp_source, lsp_source_len, rel, def_module,
                                      ...
                                      rc->def_count, imp_keys, lsp_imp_vals, imp_count,
            ...
            /* Free the on-demand re-read on both success and short-read paths
             * (no-op when source was retained in the result arena). */
...
int cbm_parallel_resolve(cbm_pipeline_ctx_t *ctx, const cbm_file_info_t *files, int file_count,
    ...
    if (!ctx || file_count < 0 || worker_count <= 0 || def_count < 0 ||
        (file_count > 0 && (!files || !result_cache || !shared_ids || !ctx->cancelled ||
                            !ctx->gbuf || !ctx->registry))) {
        return CBM_NOT_FOUND;
    }
    ...
    atomic_init(&rc.worker_failed, 0);
    ...
    int merge_rc = 0;
    bool worker_failed = atomic_load_explicit(&rc.worker_failed, memory_order_relaxed) != 0;
    for (int i = 0; i < worker_count; i++) {
        if (workers[i].local_edge_buf) {
            int worker_merge_rc =
                worker_failed ? 0 : cbm_gbuf_merge(ctx->gbuf, workers[i].local_edge_buf);
            if (worker_merge_rc != 0) {
                merge_rc = worker_merge_rc;

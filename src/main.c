...
static void *watcher_thread(void *arg) {
    /* Fork: configurable base/max poll intervals passed via watcher_thread_args_t
     * (read from config "watcher_poll_base_ms"/"watcher_poll_max_ms" at start_watcher).
     * Upstream used a hardcoded WATCHER_BASE_INTERVAL_MS 5000; the fork's args struct
     * is the superset and is what start_watcher() now passes. */

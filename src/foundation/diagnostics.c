...
static void write_diagnostics(void) {
    ...
    /* Written through the private diagnostics directory and published by
     * rename; the process CPU fields come from the same mi_process_info call
     * that already supplies the memory counters. */
    ...
    int length = snprintf(snapshot, sizeof(snapshot),
                          ...
                          "  \"pid\": %d\n"
                          "}\n",
                          uptime, user_ms, sys_ms, current_rss, peak_rss, current_commit,
                          peak_commit, page_faults, fds, qcount, qerrors, qtime, qavg, qmax,
                          (int)getpid());

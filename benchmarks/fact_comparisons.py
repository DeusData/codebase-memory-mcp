...
SCHEMA_VERSION = 1
SCHEMA_URI = "benchmarks/schema/comparisons-v1.schema.json"
FACT_SCHEMA_URIS = {
    1: {"docs/schema/benchmark-facts-v1.schema.json"},
    2: {
        "benchmarks/schema/facts-v2.schema.json",
...
TERMINOLOGY_PATH = Path(__file__).with_name("terminology.json")
...
def portable_source_path(path: Path) -> str:
    ...
    try:
        ...
        parent_id = content_id(str(resolved.parent), length=12)
        return f"external/{parent_id}/{resolved.name}"


def portable_value(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: portable_value(child) for key, child in value.items()}
    if isinstance(value, list):
        return [portable_value(child) for child in value]
    if isinstance(value, str):
        if Path(value).is_absolute():
            return portable_source_path(Path(value))
        windows_path = PureWindowsPath(value)
        if windows_path.is_absolute():
            parent_id = content_id(str(windows_path.parent), length=12)
            return f"external/{parent_id}/{windows_path.name}"
    return value
...
def load_fact_bundle(path: Path) -> dict[str, Any]:
    ...
    expected_uris = FACT_SCHEMA_URIS.get(version)
    if expected_uris is None or document.get("$schema") not in expected_uris:
...
def aggregate_cells(
    ...
    for path, bundle in sources:
        ...
        group["source_fact_paths"].append(portable_source_path(path))
...
def generate_comparison_document(
    ...
    *,
    ...
    return {
        ...
        "cell_groups": portable_value(cells),

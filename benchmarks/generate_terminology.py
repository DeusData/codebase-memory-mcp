...
def render_header(registry: dict[str, Any]) -> str:
    ...
    macro_lines = ["#define CBM_BENCHMARK_STEP_IDS(X)"]
    macro_lines.extend(
        f'    X({term_id.upper()}, "{term_id}")' for term_id in step_ids
    )
    # Emit the repository's clang-format-stable escaped-newline layout so the
    # generator check and format gate remain composable on every platform.
    continuation_width = max(len(line) for line in macro_lines[:-1])
    rows = " \\\n".join(
        line.ljust(continuation_width) for line in macro_lines[:-1]
    )
    rows += f" \\\n{macro_lines[-1]}"
    return (
        "/* Generated by benchmarks/generate_terminology.py; do not edit. */\n"

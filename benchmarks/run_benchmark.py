...
BENCHMARK_TERMINOLOGY_MARKDOWN_PATH = (
    Path(__file__).resolve().parents[1] / "docs" / "BENCHMARK_TERMINOLOGY.md"
)
...
BENCHMARK_FACT_COMPATIBLE_SCHEMA_URIS = {
    BENCHMARK_FACT_SCHEMA,
    "docs/schema/benchmark-facts-v2.schema.json",
}
...
def validate_benchmark_facts(
    facts: dict[str, Any], *, require_current_contract: bool = True
) -> None:
    if facts.get("schema_version") != BENCHMARK_FACT_SCHEMA_VERSION:
        raise ValueError("benchmark facts schema_version is unsupported")
    terminology_version = facts.get("terminology_version")
    if not isinstance(terminology_version, str) or not re.fullmatch(
        r"[1-9]\d*\.\d+\.\d+", terminology_version
    ):
        raise ValueError("benchmark facts terminology_version must be semantic")
    terminology_sha256 = facts.get("terminology_sha256")
    if not isinstance(terminology_sha256, str) or not re.fullmatch(
        r"[0-9a-f]{64}", terminology_sha256
    ):
        raise ValueError("benchmark facts terminology_sha256 must be a SHA-256")
    if require_current_contract and (
        terminology_version != BENCHMARK_TERMINOLOGY_VERSION
        or terminology_sha256 != BENCHMARK_TERMINOLOGY_SHA256
    ):
        raise ValueError(
            "benchmark facts terminology contract does not match the current registry"
        )
...
def load_benchmark_fact_bundle(path: Path) -> dict[str, Any]:
    ...
    if version == BENCHMARK_FACT_SCHEMA_VERSION:
        if document.get("$schema") not in BENCHMARK_FACT_COMPATIBLE_SCHEMA_URIS:
            raise ValueError(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "benchmarks/terminology.json), then exit."
        ),
    )
    parser.add_argument(
        "--candidate-revision",
